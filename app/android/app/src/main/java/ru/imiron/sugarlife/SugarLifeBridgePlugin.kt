...
class SugarLifeBridgePlugin : Plugin() {
    ...
    override fun handleOnResume() {
        ...
        /* И ПЕРЕЧИТАТЬ РАЗРЕШЕНИЯ (#685). Возврат на экран — это и возврат из системного запроса
           разрешений: диалог перекрывает наше окно, а закрывшись, отдаёт его обратно. Значит здесь
           же и надо узнать, что человек только что нажал «Разрешить».

           Довод тот же, что строкой выше и в systemStateWatcher ниже: исправив причину по нашей
           подсказке, человек не должен продолжать читать ту же подсказку. */
        SugarLifeService.пересмотреть(context.applicationContext)
    }
    ...
    /* ОТВЕТ НА СИСТЕМНЫЙ ДИАЛОГ — СОБЫТИЕМ, А НЕ ПРИ СЛЕДУЮЩЕМ ЗАХОДЕ (#558).

       Замечание владельца: разрешение дал, а строка в разделе осталась прежней, пока не выйдешь и
       не зайдёшь снова. Так и было: экран перечитывал состояние по возвращении из фона, а системный
       диалог разрешений в фон приложение НЕ уводит — он висит поверх той же активности.

       Получалось, что человек делает ровно то, о чём мы просили, и не видит никакого следа. Дальше
       он либо жмёт «спросить» ещё раз (а второго диалога уже не будет), либо решает, что не
    ...
    override fun handleRequestPermissionsResult(
        ...
        runCatching {
            notifyListeners("разрешения", JSObject().put("список", собратьРазрешения()))
        }.onFailure { Log.w(TAG, "ответ о разрешениях не доехал: ${it.message}") }
    }
    ...
    private fun собратьРазрешения(): JSONArray {
        ...
        /* БАТАРЕЯ: НУЖНОСТЬ РЕШАЕТ ЯДРО, А НЕ МЫ (#558).

           Замечание владельца: «зачем нам повышенное энергопотребление, если мы висим на сенсоре?».
           Вопрос верный — исключение из энергосбережения нужно не всегда. При прямом сенсоре по
           Bluetooth служба переднего плана держится и без него; дорого оно становится там, где
           данные идут из облака (в Doze режется сеть) или где прошивка чистит память по-своему.

           Судить об этом сами мы не станем: у ядра для этого есть `BackgroundReadiness.advise`, где
           учтён производитель телефона, и там же лежат слова — одни на обе платформы и оба издания.
           Красим красным, только когда ядро говорит, что проблема есть; иначе это «не выдано, и
           ладно», а не беда. Постоянная красная точка учит не замечать красные точки. */
        val pm = ctx.getSystemService(Context.POWER_SERVICE) as android.os.PowerManager
        val батарея = pm.isIgnoringBatteryOptimizations(ctx.packageName)
        val советПоФону = ru.imiron.sugarlife.contract.BackgroundReadiness.advise(
            manufacturer = android.os.Build.MANUFACTURER,
            exemptFromBatteryOptimization = батарея,
        )
        список.put(
            пункт("батарея",
                if (батарея) "разрешено" else if (советПоФону.problem) "нет" else "частично",
                false)
                /* Слова ядра отдаём как есть: они точнее наших — знают про вендорские прошивки. */
                .put("пояснение", советПоФону.reason),
        )
    ...
    /**
     * ЧТО НАЗНАЧИЛ ДВИЖОК — ТО И ДЕРЖИМ (SugarLifeCore#217).
     *
     * Раньше опора работала одинаково всегда, по галочке человека. Замерено, чем это обходится:
     * около десятой части всего разряда и телефон БЕЗ Doze вовсе — двое суток без единого
     * засыпания. И там же замерено, что от Huawei она не спасает. Платить одинаково везде — значит
     * платить зря там, где не нужно, и не спасать там, где нужно.
     *
     * Разбираем одно поле подстрокой, а не всем JSON: снимок весит тридцать килобайт и приходит на
     * каждое изменение состояния, а нам нужно короткое слово. Полный разбор ради него мы платили бы
     * тем же процессором, который эта правка и бережёт.
     */
    private fun применитьСтупеньОпоры(json: String) {
        val ключ = "\"anchorLevel\":\""
        val i = json.indexOf(ключ)
        if (i < 0) return                       // старое ядро без лестницы — оставляем как было
        val j = json.indexOf('"', i + ключ.length)
        if (j < 0) return
        runCatching { ЗвуковаяОпора.общая(context).ступень(json.substring(i + ключ.length, j)) }
    }
    ...
    companion object {
        ...
        /* МНЕНИЕ ТЕЛЕФОНА О СЕТИ (мост 1.69, SugarLifeCore#213, наш #805): состояние, а не признак
           жизни.

           Три факта, которых у движка нет: есть ли интернет, сколько его не было и идём ли мы через
           VPN. Слова по ним подбирает он — правило владельца «определяем мы, произносит ядро».

           ДЛИТЕЛЬНОСТЬ ШЛЁМ ОДИН РАЗ. Дыра — событие, а не состояние; повторять её при каждом
           докладе значило бы заводить движку по дыре на каждое шевеление сети.

           VPN ЗДЕСЬ ОТВЕЧАЕТ И «НЕТ». У поля три состояния, и на Android достижимы все:
           `TRANSPORT_VPN` — документированный признак, значит `false` тут это факт, а не догадка.
           На iPhone мы шлём только `true` или молчим — там туннель узнаётся по именам интерфейсов.
           Разницу ядро попросило выразить типом, а не текстом: так её не потерять при правке слов. */
        fun мнениеОСети(интернетЕсть: Boolean): String = buildString {
            append("{\"type\":\"networkBack\",\"internet\":").append(интернетЕсть)
            append(",\"vpnActive\":").append(СостояниеСети.черезVPN)
            СостояниеСети.взятьОбрыв()?.let { append(",\"offlineMs\":").append(it * 1000L) }
            append("}")
        }
    }
}
