...
class ВендорскийАлгоритм private constructor(
    ...
    companion object {
        ...
        /**
         * УМОЛЧАНИЯ У СЕМЕЙСТВА БОЛЬШЕ НЕТ (SugarLifeCore#225).
         *
         * Здесь стояло `семейство: Семейство = Семейство.V115G`. Удобство обошлось в то, что
         * `reset()` — который драйвер зовёт перед КАЖДЫМ прогревом — пересоздавал ядро по умолчанию и
         * молча менял алгоритм с V116A на V115G. В логе при этом честно стояло «семейство=V116A»:
         * строка пишется в `configure`, и в тот миг она верна.
         *
         * На приборе владельца это давало 7,9 там, где тот же алгоритм на стенде отвечал 10,6.
         * Заметили только потому, что рядом стоит чужая программа с тем же сенсором.
         *
         * Умолчание здесь — не удобство, а тихий неверный ответ. Пусть выбор требует компилятор.
         */
        fun создать(
