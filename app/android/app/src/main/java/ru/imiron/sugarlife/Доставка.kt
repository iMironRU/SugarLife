        ...
        поток.execute {
            val json = buildString {
                ...
                /* VPN И ДЛИТЕЛЬНОСТЬ ОБРЫВА — ФАКТЫ, КОТОРЫЕ ВИДИМ ТОЛЬКО МЫ (SugarLife#805).
                   Положительный ответ шлём, отрицательный нет: пустое поле честнее ложного
                   `false` там, где на соседней платформе это догадка. */
                if (СостояниеСети.черезVPN) append(""","vpnActive":true""")
                СостояниеСети.последнийОбрывСек?.let { append(""","lastOutageSec":$it""") }
                append("}")
            }
