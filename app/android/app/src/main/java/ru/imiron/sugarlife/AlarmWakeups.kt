    ...
    private suspend fun ждатьБудильник(ms: Long) {
        ...
        suspendCancellableCoroutine { продолжение ->
            ...
            bleLog(
                ...
                "мс" to ms.toString(),
                /* ПРИЧИНУ НАЗЫВАЕМ ВЕРНО (SugarLifeCore#208). Здесь стояло «нет разрешения на точные —
                   система вправе сдвинуть». На Android 10 такого разрешения не существует вовсе, и код
                   это знает (`Точность.разрешеноСистемой`: до Android 12 всегда true). Причина была в
                   НАШЕМ выключателе, выключенном по умолчанию, — а сообщение показывало на систему.
                   Разбор ночи на песочнице упёрся ровно в эту строку: она уводила от настоящей причины. */
                "будильник" to when {
                    точный -> "точный"
                    !Точность.разрешеноСистемой(ctx) -> "неточный: система не дала разрешения на точные"
                    else -> "неточный: выключатель точных пробуждений выключен у нас"
                },
            )
