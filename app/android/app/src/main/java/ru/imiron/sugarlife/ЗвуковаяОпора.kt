...
class ЗвуковаяОпора(private val ctx: Context) {
    ...
    /* Играть мало — надо ещё числиться проигрывателем (SugarLifeCore#216). Сеанс живёт ровно столько
       же, сколько звук: сеанс без звука — такая же половина дела, как звук без сеанса. */
    private val сеанс = СеансПлеера(ctx)
    private var режим: Режим = Режим.Выключено

    /**
     * СТУПЕНЬ, КОТОРУЮ НАЗНАЧИЛ ДВИЖОК (SugarLifeCore#217).
     *
     * До сих пор опора включалась галочкой человека и работала одинаково всегда. Замерено, чем это
     * обходится: около десятой части всего разряда и телефон **без Doze вовсе** — двое суток без
     * единого засыпания. И одновременно замерено, что от Huawei она не спасает.
     *
     * Значит платить одинаково везде заведомо неверно. Ступень выбирает движок по журналу жизни:
     * убивали — поднимаемся, не трогают — спускаемся. Здесь мы её только исполняем.
     */
    private var ступень: String = "Нет"

    /** Держать ли процессор. Только на верхней ступени: это она отнимает Doze. */
    private val держатьПроцессор: Boolean get() = ступень == "ЗвукИПроцессор"

    /** Играть ли звук. На ступени «Сеанс» мы числимся плеером, ничего не издавая. */
    private val нуженЗвук: Boolean get() = ступень == "Звук" || ступень == "ЗвукИПроцессор"

    /** Нужен ли сеанс плеера. Со ступени «Сеанс» и выше. */
    private val нуженСеанс: Boolean get() = ступень != "Нет"
    ...
    /**
     * Можно ли объявлять службу медиа-воспроизведением: звук идёт И мы числимся плеером.
     *
     * Оба условия обязательны. Объявить `mediaPlayback`, не играя, — то же самое враньё системе, что
     * `connectedDevice` без прибора, и наказывают за него так же (SugarLifeCore#216).
     */
    val можноОбъявлятьМедиа: Boolean get() = держимЗвук && сеанс.активен
    ...
    /**
     * Движок назначил ступень. Зовётся на каждый снимок — работу делаем только при смене.
     *
     * Галочка человека остаётся правом вето: выключил опору — значит выключил, что бы ни решил
     * движок. Обратное неверно: включённая галочка не заставляет платить, если платить не за что.
     */
    fun ступень(новая: String) {
        if (новая == ступень) return
        val было = ступень
        ступень = новая
        /* Имя разведено с движковым намеренно: там «лестница-решила» (кто ВЫБРАЛ ступень), здесь
           «опора-применена» (кто её ИСПОЛНИЛ). Прежние «опора-ступень» и «ступень опоры» отличались
           только порядком слов, и на этом сходстве я трижды подряд починил не ту причину. */
        bleLog("Info", "опора-применена", null, "было" to было, "стало" to новая)
        применить()
        /*
         * СКАЗАТЬ СЛУЖБЕ, ЧТО ПРАВДА О НАС ИЗМЕНИЛАСЬ (SugarLifeCore#217).
         *
         * Тип службы пересматривался на старте, на смене прибора и на уходе экрана в фон — но не на
         * смене ступени. Снято с телефона владельца 11.09: на переподключении опора включилась на
         * две секунды, тип поднялся до `connectedDevice+mediaPlayback`, опора погасла — а системе мы
         * ещё два с половиной часа обещали проигрыватель, которого не было:
         *
         * ```
         * 12:13:19  опора-применена: Нет → Звук
         * 12:13:21  тип службы поднят: connectedDevice → connectedDevice+mediaPlayback
         * 12:14:12  опора-применена: Звук → Нет      ← и больше тип никто не пересматривал
         * 14:48     types=0x00000012
         * ```
         *
         * Задержка та же, что везде рядом: сеансу плеера нужен тик, чтобы стать активным или погаснуть,
         * а `можноОбъявлятьМедиа` смотрит именно на него.
         */
        Handler(Looper.getMainLooper()).postDelayed({ SugarLifeService.пересмотреть(ctx) }, 1_500L)
    }
    ...
    private fun применить() {
        /* Человек выключил — выключено, чего бы ни хотел движок: это его телефон и его заряд. */
        val можно = вФоне && режим != Режим.Выключено
        if (можно && нуженСеанс) начать() else остановить()
    }

    private fun начать() {
        val интервал = режим.интервалМс ?: 5_000L
        сеанс.начать()
        /* На ступени «Сеанс» звук не издаём вовсе: она бесплатная и проверяет, хватит ли прошивке
           одного того, что мы числимся проигрывателем. Если не хватит — движок поднимет нас выше. */
        if (нуженЗвук && проигрыватель == null) собрать()
        if (!нуженЗвук) { runCatching { проигрыватель?.stop(); проигрыватель?.release() }; проигрыватель = null }
    ...
    private fun собрать() {
        ...
        проигрыватель = runCatching {
            MediaPlayer().apply {
                ...
                setVolume(0f, 0f)
                /*
                 * ПРОЦЕССОР ДЕРЖИМ ТОЛЬКО НА ВЕРХНЕЙ СТУПЕНИ (SugarLifeCore#217).
                 *
                 * Строка стояла всегда, и обоснование было разумным: «процессор не должен засыпать,
                 * пока мы играем, иначе опора держит процесс, но не даёт ему работать».
                 *
                 * Цена оказалась выше довода. Замерено на рабочем телефоне: сутки непрерывного
                 * partial wakelock и `Device deep doze discharge: 0` — телефон не заснул НИ РАЗУ за
                 * двое суток. А будильники у нас свои и процессор будят сами, так что между
                 * пробуждениями держать его незачем.
                 *
                 * Поэтому теперь это отдельная ступень, и включает её движок — когда прошивка не
                 * оставила выбора, а не на всякий случай.
                 */
                if (держатьПроцессор) setWakeMode(ctx.applicationContext, PowerManager.PARTIAL_WAKE_LOCK)
                prepare()
                start()
            }
        }.onFailure { bleLog("Warn", "опора не собралась: $it", null) }.getOrNull()
        if (проигрыватель != null) {
            bleLog("Info", "ушли в фон с опорой", null,
                "тик" to "${(режим.интервалМс ?: 0) / 1000} с", "ступень" to ступень)
        }
    }
    ...
    private fun завестиДозор(интервал: Long) {
        ...
        руки.postDelayed(object : Runnable {
            override fun run() {
                ...
                тиков++
                if (!нуженЗвук) {
                    /* На ступени «Сеанс» играть нечего — только подтверждаем, что мы плеер. */
                    сеанс.подтвердить()
                } else if (держимЗвук) {
                    if (молчалиПодряд > 0) {
                    ...
                    /* Позицию в сеансе двигаем на каждом тике: состояние, застывшее на месте, система
                       вправе счесть брошенным — и тогда мы снова перестанем быть плеером, играя. */
                    сеанс.подтвердить()
                    // Раз в пять минут — живая строка: иначе утром не отличить «работала» от «не запускалась».
                    if (тиков % maxOf(1L, 300_000L / интервал) == 0L) {
                        bleLog("Debug", "опора держит звук", null,
                            "тиков" to тиков.toString(), "плеером" to сеанс.активен.toString())
                    }
                } else {
