...
object ТревогиДвижка {
    ...
    fun приСнимке(ctx: Context, json: String): Boolean {
        ...
        if (ведёт && !дежуритДвижок) {
            ...
            Тревоги.уступить(ctx)
        }
        if (!ведёт && дежуритДвижок) {
            /* Откат ядра на сборку без модели. Дежурство возвращается нам — вместе со сторожем
               молчания, который при передаче был отменён. */
            Log.i(TAG, "движок перестал вести тревоги — возвращаем дежурство себе")
            показано.keys.toList().forEach { снять(ctx, it) }
            Тревоги.вернутьДежурство(ctx)
        }
        дежуритДвижок = ведёт
