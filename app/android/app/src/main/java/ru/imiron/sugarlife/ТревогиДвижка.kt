...
object ТревогиДвижка {
    ...
    fun приСнимке(ctx: Context, json: String): Boolean {
        ...
        /* `optString(name, null)` компилятор принимал, но предупреждал: у метода второй параметр
           объявлен ненулевым, и мы передавали туда null. Работало, но держалось на том, что
           платформенный тип не проверяется. `ведётДвижок` умеет принять null сам — так и скажем,
           отсутствие поля означает «ревизии нет», а не пустую строку. */
        val ревизия = if (снимок.has("bridgeRevision")) снимок.optString("bridgeRevision") else null
        val ведёт = ПравилаПоказа.ведётДвижок(ревизия, правил)
    ...
    private fun показать(ctx: Context, с: ПравилаПоказа.Событие, как: ПравилаПоказа.Как) {
        ...
        /* В лог — ПРИКАЗ, а не наше толкование (то же, что на iOS). Когда ночью тревога прозвучит
           не так, вопрос будет один: движок велел или мы не исполнили. По этой строке видно сразу. */
        Log.i(TAG, "тревога ${с.id} — ${с.loudness}/${с.soundKind}, звучать ${с.sound}, " +
            "пробиваться ${с.bypassQuiet}, полный экран ${с.fullScreen}, " +
            "громкость до ${с.raiseVolumeTo ?: "—"}${if (с.inCar) ", в машине" else ""} → канал $канал")
