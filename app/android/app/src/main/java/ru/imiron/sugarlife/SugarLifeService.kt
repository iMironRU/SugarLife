...
class SugarLifeService : Service() {
    ...
    override fun onCreate() {
        ...
        /*
         * ЗВУКОВАЯ ОПОРА (SugarLifeCore#123) — только если человек её включил.
         *
         * На стоковом Android она не нужна: служба переднего плана и так переживает дремоту. Нужна
         * против прошивок, которые выгружают приложения по своему усмотрению, — воспроизведение они
         * обычно щадят. Цена: телефон считает нас источником звука.
         */
        if (звуковаяОпораВключена()) ЗвуковаяОпора.начать(applicationContext)
     ...
     */
    /** Настройка человека: держаться звуком. По умолчанию выключено — на стоковом телефоне лишнее. */
    private fun звуковаяОпораВключена(): Boolean =
        getSharedPreferences("sugarlife-alarms", Context.MODE_PRIVATE).getBoolean("audioKeepalive", false)
    ...
    /**
     * Отпустить звук при остановке службы.
     *
     * Обязательно: пока мы держим воспроизведение, магнитола считает нас источником звука, и человек
     * не поймёт, почему музыка не играет с телефона (SugarLifeCore#123).
     */
    override fun onDestroy() {
        ЗвуковаяОпора.прекратить()
        super.onDestroy()
    }
