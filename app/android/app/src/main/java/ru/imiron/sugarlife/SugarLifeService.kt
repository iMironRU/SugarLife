...
class SugarLifeService : Service() {
    ...
    override fun onCreate() {
        super.onCreate()
        живаяСлужба = true
        /* Сторож главного потока (SugarLifeCore#236). Заводится здесь, а не в экране: поток у
           процесса один, а служба живёт дольше всех — значит следить будет и тогда, когда экрана нет. */
        ...
        /* На EMUI виджет — единственный путь назад после остановки пакета. Пишем в журнал при каждом
           старте: без этой строки нельзя отличить «нас не подняли» от «поднимать было нечем». */
        Log.i(TAG, if (Воскрешение.виджетЕсть(applicationContext)) "виджет на экране — путь назад есть"
                   else "ВИДЖЕТА НА ЭКРАНЕ НЕТ — после остановки пакета поднять нас будет нечем")
    ...
    private fun слушатьРадиТревог() {
        runCatching {
            EngineHolder.engine(applicationContext).subscribe { json ->
                ...
                отметитьСвязьПриборов(json)   // чем представляемся системе — по факту (#228)
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        when (intent?.action) {
            СПРОСИТЬ -> спроситьТочноЛи()
            ОТДАТЬ -> отдатьПриборы("подтвердили в уведомлении")
            ВЗЯТЬ -> взятьПриборы()
        }
        return START_STICKY
    }
     ...
     */
    /**
     * ДЕРЖИМ ЛИ ПРИБОР ПРЯМО СЕЙЧАС (SugarLifeCore#228).
     *
     * Берётся из снимка движка — из того же поля, что видит человек в карточке. Пока снимка не было,
     * считаем, что не держим: обещать системе прибор до первого подтверждения значит начинать с того
     * самого вранья, за которое нас и убивали.
     */
    @Volatile private var приборНаСвязи = false

    private fun выбратьТип(): Int = ТипСлужбы.выбрать(
        прибор = приборНаСвязи && bluetoothРазрешён(this),
    ...
    fun поправитьТип() {
        ...
        if (ТипСлужбы.поднимать(текущийТип, нужен)) {
            runCatching {
     ...
     */
    private fun отметитьСвязьПриборов(json: String) {
        val было = приборНаСвязи
        приборНаСвязи = runCatching {
        ...
        if (приборНаСвязи != было) поправитьТип()
    }
    ...
    private fun сахарИзСнимка(json: String): String? {
        ...
        /* ЗАСТАВКЕ ЧИСЛО НУЖНО РАНЬШЕ, ЧЕМ МЫ ЕГО ПОЛУЧИМ (SugarLifeCore#232). Здесь оно живёт в
           памяти и умирает вместе с процессом, а нативная заставка рисуется, когда процесс только
           поднялся. Кладём в хранилище те же три величины, что показывает уведомление, — чтобы не
           завести вторую копию правила о том, что показывать. */
        if (когда > 0) ЗаставкаЧисла.запомнить(this, сахар, когда, сырое = !калиброван)
    ...
    /**
     * Спрашиваем ли мы сейчас «точно отдать?». Живёт до ответа или до [ПЕРЕДУМАЛИ_МС].
     *
     * Вопрос обязан сам себя снимать: уведомление постоянное, и застрявшее в нём «Точно отдать?»
     * человек увидит через час, уже не помня, что он нажимал. Тогда второе касание отдаст приборы
     * ровно так же неожиданно, как отдавало первое до этой правки.
     */
    private var спрашиваем = false
    private var снятиеВопроса: Runnable? = null

    private fun спроситьТочноЛи() {
        спрашиваем = true
        показать(держим = держимПриборы)
        снятиеВопроса?.let { руки.removeCallbacks(it) }
        снятиеВопроса = Runnable {
            if (!спрашиваем) return@Runnable
            спрашиваем = false
            показать(держим = держимПриборы)
        }.also { руки.postDelayed(it, ПЕРЕДУМАЛИ_МС) }
    }
    ...
    private fun уведомление(держим: Boolean = true): Notification {
        ...
        val действие = when {
            !держим -> ВЗЯТЬ
            спрашиваем -> ОТДАТЬ      // вопрос уже задан: это касание подтверждает
            else -> СПРОСИТЬ          // первое касание только переспрашивает
        }
        /* Свой код запроса на каждое действие: с общим PendingIntent один перезаписывал бы другой, и
           кнопка делала бы не то, что на ней написано. */
        val код = when (действие) { ВЗЯТЬ -> 2; ОТДАТЬ -> 3; else -> 1 }
        val кнопка = PendingIntent.getService(
            this, код, Intent(this, SugarLifeService::class.java).setAction(действие),
        ...
        return NotificationCompat.Builder(this, CHANNEL)
            ...
            .setStyle(
                NotificationCompat.BigTextStyle().bigText(
                    if (держим) {
                        /* ЦЕНУ НАЗЫВАЕМ ДО, А НЕ ПОСЛЕ. Человек, уже нажавший, читает второй текст —
                           и решает по нему, нажимать ли снова. Без слова «наблюдение остановится»
                           вопрос «Точно отдать?» звучит как формальность. */
                        if (спрашиваем) {
                            "Наблюдение остановится: числа перестанут приходить, пока прибор не " +
                                "освободится. Нажмите ещё раз, чтобы отдать, или подождите — вопрос снимется сам."
                        } else {
            ...
            .addAction(
                0,
                when {
                    !держим -> "Взять обратно"
                    спрашиваем -> "Точно отдать?"
                    else -> "Отдать приборы"
                },
                кнопка,
            )
    ...
    companion object {
        ...
        /* ПЕРВОЕ КАСАНИЕ НЕ ОТДАЁТ (SugarLifeCore#220, решение владельца).
        
           «Отдать приборы» останавливает наблюдение насовсем — до этой правки одним касанием и без
           единого вопроса, кнопкой, стоящей прямо под числом сахара, то есть там, куда человек
           смотрит весь день. На телефоне владельца это стоило сорока восьми минут тишины.
        
           Везде у нас такое подтверждается удержанием (`HoldButton`, 2 с). В шторке удержания нет:
           система знает только касание. Поэтому здесь два шага — первое касание переспрашивает,
           второе отдаёт. Смысл тот же: случайное касание не должно ничего останавливать. */
        /** Сколько висит вопрос «Точно отдать?», прежде чем снимется сам. */
        private const val ПЕРЕДУМАЛИ_МС = 15_000L
        private const val СПРОСИТЬ = "ru.imiron.sugarlife.ASK_RELEASE"
