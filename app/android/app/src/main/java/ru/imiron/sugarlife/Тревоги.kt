...
object Тревоги {
    ...
    /* ЧЬЁ ЭТО УВЕДОМЛЕНИЕ — ВИДНО НА ЧАСАХ (#701, решение владельца).

       На телефоне имя приложения показывает система; на часах его часто нет, и «Низкий сахар»
       неотличим от такой же тревоги соседнего приложения, читающего то же облако. У человека с
       диабетом таких приложений два-три, и говорят они об одном сахаре.

       Буквы, а не значок: значок на чужом экране может не доехать. Ставим `setSubText` — ближайшее
       к айфонному подзаголовку: идёт в шапке рядом с именем приложения и не съедает ширину
       заголовка, ради которой всё и затевалось. */
    const val МЕТКА_ОТПРАВИТЕЛЯ = "SL"
    ...
    private fun показать(ctx: Context, заголовок: String, текст: String, id: Int = ID_ГИПО,
        ...
        val n: Notification = NotificationCompat.Builder(ctx, КАНАЛ)
            /* Метка отправителя (#701): заголовки не трогаем — «Тревога может не разбудить»
               написана так, чтобы не читаться как тревога про сахар (#482). */
            .setSubText(МЕТКА_ОТПРАВИТЕЛЯ)
