...
object Тревоги {
    ...
    /**
     * ВЕРНУТЬ ДЕЖУРСТВО СЕБЕ (#482).
     *
     * Случай редкий, но не выдуманный: откат ядра на сборку без модели тревог. Снимок перестаёт нести
     * правила — и если бы мы просто «перестали молчать», сторож молчания остался бы отменённым, а
     * заметно это стало бы только той ночью, когда сенсор отвалится.
     */
    fun вернутьДежурство(ctx: Context) {
        SilenceWatchdog.поНастройке(ctx)
        Log.i(TAG, "движок больше не ведёт тревоги — дежурим снова мы")
    }
