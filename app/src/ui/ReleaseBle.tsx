...
/* УДЕРЖАНИЕ, А НЕ КАСАНИЕ (#825, решение владельца: «спрашивать подтверждение, удержанием как и
   везде»).

   6 сентября на телефоне владельца прибор трижды переставал давать данные, и один раз это стоило
   СОРОКА ВОСЬМИ МИНУТ ТИШИНЫ. Каждый раз приходил `releaseBle`, а смахивания при этом не было:
   движок пишет `остановка-объявлена` только у двух случаев из пяти. Слать эту команду умеют ровно
   двое — кнопка в шторке и эта. В шторке удержания нет, там система знает только касание, и там
   уже сделано два шага с вопросом.

   Кнопка, останавливающая наблюдение одним касанием, стояла на первом экране среди обычных.

   Цену называем ДО, а не после: «наблюдение остановится» — это то, ради чего удержание и заведено,
   и человек должен прочесть это раньше, чем додержит. */
...
export default function ReleaseBle() {
  ...
  const покой = фаза === 'idle';
  return (
    <div className={'release-ble' + вид.cls}>
      ...
      <span>
        ...
        <i>
          {фаза === 'готово'
            ...
            /* Сначала цена, потом польза: останавливается наблюдение, и это главное из двух. */
            : `Наблюдение остановится: числа перестанут приходить, пока прибор не освободится. `
              + `Отпустить ${bleУстройства.length === 1 ? 'подключение' : 'подключения'} по блютусу — чтобы сенсор увидел другой телефон.`}
        </i>
        {/* Подпись кнопки короче заголовка намеренно: «Отдать устройства» уже стоит строкой выше,
            и повторённое дословно оно читается как заикание, а не как действие. */}
        {покой && (
          <HoldButton label="Отдать" className="release-ble-hold" holdMs={2000}
            onComplete={() => void отдать()} />
        )}
      </span>
    </div>
  );
