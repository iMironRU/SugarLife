...
export default function Screen({ tab, panel = 'compact', обновить, безСередины, children }: {
  tab: number;
  ...
  panel?: 'full' | 'compact';
  /* ПАНЕЛЬ НЕ ЕЗДИТ ПОД ПЕРЕКЛЮЧАТЕЛЕМ РАЗДЕЛОВ (SugarLife#734).

     Правило середины писалось про ЭКРАНЫ: на «Инсулине» под коротким графиком остаётся
     половина экрана пустоты, и сжимать панель там не за что. Внутри одного экрана с
     тремя разделами оно означает другое: «Анализ» короткий, «Метрики» длинные, и
     переключая раздел, человек двигает панель над собой — хотя экран не менял и
     панель не трогал.

     Владелец назвал это «прыгает шапка», и он прав: шапка обязана быть одна на экран.
     Цена — под коротким разделом остаётся пустое место; это дешевле, чем орган
     управления, уезжающий на сорок шесть пикселей от чужого нажатия. */
  безСередины?: boolean;
  children: ReactNode;
}) {
  ...
  useEffect(() => {
    if (panel !== 'compact' || безСередины) return;
  ...
  return (
    <IonPage>
      <IonContent ref={ref} fullscreen forceOverscroll scrollEvents onIonScroll={reportContentScroll}>
        {обновить && (
          /* ПОРОГ ВЕРНУЛИ К ОБЫЧНОМУ (просьба владельца: «как сильно нужно оттянуть?»).

             Стояло девяносто вместо ионовских шестидесяти, с доводом «панель ездит за пальцем, и
             случайный потяг при обычной прокрутке запускал бы опрос по десять раз». Довод был про
             экран, забитый содержимым.

             На живом телефоне «Сегодня» наполовину пуст: прокручивать нечего, случайно потянуть
             невозможно, а порог остался как для плотного экрана. Снаружи это читается не как
             «тугой жест», а как «жеста нет»: человек тянет на обычную величину, ничего не
             происходит, и он делает вывод, что обновления тут не предусмотрено.

             Невидимая функция и отсутствующая — одно и то же. Возвращаем шестьдесят: ложное
             срабатывание стоит одного лишнего опроса, а ненайденный жест — веры в то, что
             приложение вообще умеет обновляться. */
          ...
          <IonRefresher slot="fixed" pullMin={60} pullMax={200}
            onIonRefresh={(e) => {
              setМолчат(false);
              void (async () => {
