...
import {
  ЦЕЛИ, строкиНМГ, полнота, вопросыДляВрача, динамика, сравнимо, чтоСталоЛучше,
  МИН_ДНЕЙ, МИН_ДОЛЯ, type НаборЦелей,
} from '@/domain/visitNote';
...
export default function VisitNoteSection({ onClose }: { onClose: () => void }) {
  ...
  /* Предыдущие две недели — окно ТОЙ ЖЕ длины. Сравнивать две недели с месяцем нельзя:
     в длинном окне больше всего, включая плохое, и «улучшение» вышло бы из арифметики. */
  const прошлые = useMemo(
    () => три.filter((e) => e.t >= сейчас - 28 * ДЕНЬ && e.t < сейчас - 14 * ДЕНЬ),
    [три, сейчас],
  );
  const sПрошлые = useMemo(() => stats(прошлые), [прошлые]);
  const пПрошлые = useMemo(
    () => полнота(прошлые, сейчас - 28 * ДЕНЬ, сейчас - 14 * ДЕНЬ), [прошлые, сейчас],
  );
  const можноСравнить = сравнимо(п, пПрошлые);
  const сдвиги = можноСравнить ? динамика(s14, sПрошлые) : [];
  const похвала = чтоСталоЛучше(сдвиги);
  ...
  return (
    <Section title="Записка к приёму" subtitle="Что показать и о чём спросить" onBack={onClose}>
      ...
      {!можноСравнить ? (
        /* Молчать нельзя: человек, увидевший пустоту, решит, что ничего не изменилось,
           а мы просто не знаем. Дырявый прошлый период — главная ловушка сравнения:
           «время в диапазоне выросло» может значить, что пропущены были плохие часы. */
        <div className="sheet-note">
          Сравнить не с чем: в одном из периодов сенсор молчал больше трети времени.
          Такое сравнение показало бы не изменения, а пропуски.
        </div>
      ) : !сдвиги.length ? (
        <div className="sheet-note">
          Заметных изменений нет — всё держится примерно на том же уровне, что и две
          недели назад.
        </div>
      ) : (
        <>
          {похвала && (
            <div className="sheet-note good">
              Стало лучше: {похвала}. Это результат двух недель ежедневной работы —
              её стоит показать врачу.
            </div>
          )}
          <div className="basal-rows">
            {сдвиги.map((с) => (
