...
import { stats, agp } from '@/domain/agp';
...
export default function VisitNoteSection({ onClose, встроенный }: {
  ...
  /* Типовой день — та же выборка и тот же расчёт, что у графика в «Метриках»: врач и
     человек должны смотреть на одну картинку, а не на две похожие. */
  const типовойДень = useMemo(() => agp(две, 48), [две]);
  ...
  const тело = (
    <>
      ...
      {/* ТИПОВОЙ ДЕНЬ — сразу за цифрами сенсора (#156).

          Открытым вопросом было, нужен ли он в записке вовсе: график есть в «Метриках»,
          а записку смотрят с телефона, и лишний экран прокрутки может стоить дороже
          пользы. Ответ дал сам вопрос врача: проценты говорят, СКОЛЬКО времени сахар
          был не там, и молчат о том, КОГДА. Второе в кабинете спрашивают первым — и
          отвечать на это, листая в другой раздел, значит тратить те самые пятнадцать
          минут.

          Стоит именно здесь, а не в начале: сначала полнота данных (иначе картинку
          прочитают как правду о двух неделях, которых не было), потом проценты, потом
          картинка, которая их объясняет. */}
      {типовойДень.length > 0 && (
        <>
          <div className="section-label sec">Типовой день за 14 дней</div>
          <div className="agp-card"><AgpSvg точки={типовойДень} /></div>
          <div className="sheet-note">
            Линия — медиана: типичный сахар для этого часа. Плотная полоса — половина
            всех значений (25–75 %), бледная — почти все (5–95 %). Полоса между 3,9 и
            10,0 — целевой диапазон. Разрыв линии означает, что показаний за это время
            суток не было.
          </div>
        </>
      )}
  ...
  const документ = (
    <КПечати имя={имя.trim() || null} отМс={сейчас - 14 * ДЕНЬ} доМс={сейчас}
      ...
      типовойДень={типовойДень} блоки={блоки} вопросы={вопросыВФайл} />
  );
