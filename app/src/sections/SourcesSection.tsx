...
export default function SourcesSection({ onClose, встроенный }: {
  ...
  const тело = (
    <>
      ...
      {/* КУДА УХОДЯТ ДАННЫЕ — ЗДЕСЬ ЖЕ, НО ОТДЕЛЬНЫМ СПИСКОМ (#547).

          Nightscout бывает двух ролей сразу: источник и приёмник. Про первую экран говорил всегда,
          вторая была невидима — доезжают ли записи, человек узнавал случайно и поздно, вроде того
          момента, когда врач открывает отчёт и не находит там последней недели.

          Отдельным списком, а не строкой внутри слота: слот отвечает «откуда цифры», и смешивать
          с ним «куда мы пишем» значило бы снова отвечать на два вопроса одним местом. */}
      {(() => {
        const доставка = строкиДоставки(snap?.connectors, Date.now());
        if (!доставка.length) return null;
        return (
          <>
            <div className="section-label sec">Куда уходят данные</div>
            <div className="list">
              {доставка.map((д) => (
                <div key={д.id} className="list-row охрана-пункт">
                  <span className={'охрана-знак-п ' + (д.знак === 'ок' ? 'ок' : д.знак === 'нет' ? 'нет' : 'незнаем')} />
                  <span className="pick-main">
                    <span className="list-title">{д.имя}</span>
                    <span className="pick-sub">{д.под}</span>
                  </span>
                </div>
              ))}
            </div>
