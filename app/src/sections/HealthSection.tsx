...
export default function HealthSection({ onClose }: { onClose: () => void }) {
  ...
  const выбранная = список.find((с) => с.проверка.id === проверка) ?? null;
  ...
  return (
    <Section title="Здоровье" subtitle="Профиль · Здоровье" onBack={onClose}>
      ...
      <Sheet isOpen={!!выбранная} onClose={() => setПроверка(null)}
        title={выбранная?.проверка.что ?? ''} subtitle="Когда делали последний раз">
        {выбранная && (
          <>
            <div className="lim-kid">
              По рекомендациям: <b>{выбранная.проверка.цитата}</b>.
            </div>
            <div className="field-label" style={{ marginTop: 12 }}>Дата</div>
            <div className="field">
            ...
            <button className="changed-btn" style={{ marginTop: 12 }}
              onClick={() => { отметитьПроверку(выбранная.проверка.id, Date.now()); setПроверка(null); }}>
              <IonIcon icon={checkmarkOutline} /> Сделал сегодня
            </button>
            {выбранная.когда && (
...
function ЗамерШторка({ что, onClose, h }: {
  ...
  const было = что === 'вес' ? h.вес : что === 'hba1c' ? h.hba1c : null;

  return (
    <Sheet isOpen={!!что} onClose={закрыть} title={заголовок} subtitle="Записываем с сегодняшней датой"
      footer={<div className="page-foot"><button className="page-btn bas-go" onClick={сохранить}>Сохранить</button></div>}>
      {что === 'давление' ? (
        <>
        ...
        <>
          ...
          {было && <div className="field-hint">Прошлое значение: {было.значение} · {дата(было.когда)}</div>}
        </>
      )}
    </Sheet>
  );
