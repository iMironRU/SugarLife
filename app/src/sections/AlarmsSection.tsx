...
export default function AlarmsSection({ onClose }: { onClose: () => void }) {
  ...
  return (
    <Section title="Тревоги" описание="Что должно разбудить и при каких числах. Тревога — это то, что происходит, когда вы не смотрите на экран." onBack={onClose}>
      ...
      {/* Правил нет вовсе — говорим прямо. Пустой экран читается как «всё выключено», и человек
          уходит спать с уверенностью, что настроил. */}
      {можем && !правила?.length && (
        <div className="sheet-note warn">
          Движок не отдаёт правил тревог: считать их некому, и разбудить нас некому тоже. Обновите
          приложение целиком — не только интерфейс.
        </div>
      )}
