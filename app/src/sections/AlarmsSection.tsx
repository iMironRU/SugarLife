...
export default function AlarmsSection({ onClose }: { onClose: () => void }) {
  ...
  /* КОГДА ТРЕВОГИ ВЕДЁТ ДВИЖОК, НАШИ ПОРОГИ УХОДЯТ С ЭКРАНА (#482).

     Не ради чистоты: числа ниже правят настройки нашей андроидной половины, а она с этого момента
     ничего не решает. Оставить их значило бы дать человеку крутить ручки, ни к чему не подключённые, —
     и он бы уверенно ушёл спать с порогом, который никто не читает.

     Признак тот же, что и в нативной части: непустые правила в снимке. */
  const снимок = useSnapshot();
  const правила = снимок?.alarmRules;
  const ведётДвижок = !!правила?.length;
  ...
  return (
    <Section title="Тревоги" описание="Что должно разбудить и при каких числах. Тревога — это то, что происходит, когда вы не смотрите на экран." onBack={onClose}>
      ...
      {ведётДвижок && <ПравилаДвижка правила={правила} />}
