  ...
  return (
    <Section title="О приложении" onBack={onClose}
      ...
      <div className="list">
        ...
        {isNative && (
          <>
            {/* «ВОЗВРАТ К ИСХОДНОМУ», А НЕ «ЕСЛИ НЕ ОБНОВЛЯЕТСЯ» (замечание владельца).

                Заголовок обещал поломку. Блок стоит на экране всегда, и постоянная надпись «если
                интерфейс не обновляется» читается как «он у нас не обновляется» — то есть экран
                сам говорит, что с приложением что-то не так, даже когда всё в порядке.

                А действие законное и не аварийное: вернуться к тому, что внутри установленного
                приложения. Так и назовём. Причина, по которой это вообще может понадобиться,
                осталась — но объяснением, а не заголовком раздела. */}
            <div className="section-label sec">Возврат к исходному</div>
            {/* НЕ СТРОКА СПИСКА, А БЛОК (замечание владельца: «некрасиво и непонятно»).

                Строкой это не влезало: кнопка удержания несёт под собой ещё и подпись «удерживать
                2 секунды», и в ряду она распирала всё, а объяснение обрывалось на «и...». Объяснение
                здесь как раз главное — без него кнопка выглядит опасной наугад.

                Тот же вид, что у «Отдать устройства» на «Сегодня»: заголовок, объяснение, действие
                под ними во всю ширину. Он уже проверен на телефоне и читается. */}
            <div className="встроенный">
              <b>Вернуться к интерфейсу, который лежит внутри приложения</b>
              <i>
                Пригодится в двух случаях: если обновление приехало сломанным или если новый
                интерфейс ставят проводом, а он не появляется. Второе бывает потому, что
                скачанный по воздуху интерфейс главнее: пока он стоит, приложение поднимается
                из него. Кнопка возвращает исходный и перезапускает приложение — данные и
                настройки остаются.
              </i>
              <HoldButton label="Вернуть встроенный" className="встроенный-кнопка" holdMs={2000}
