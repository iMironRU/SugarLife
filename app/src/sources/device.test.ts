...
describe('сборка состояния помпы', () => {
  ...
  it('устаревший цикл не всплывает: лучше пусто, чем старое как текущее', () => {
    ...
    // но КОГДА цикл считал в последний раз — знаем: на этом держится «цикл молчит 40 мин»
    expect(d?.loopAt).toBe(t);
  });

  it('время расчёта цикла берётся из документа цикла, а не из последнего', () => {
    const d = normDeviceDocs([помпаДок(t + 5 * МИН, 75), циклДок(t, 4.085)]);
    expect(d?.at).toBe(t + 5 * МИН);
    expect(d?.loopAt).toBe(t);
  });
