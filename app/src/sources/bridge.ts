...
export interface DeviceView {
  ...
  status?: SourceStatus;
  latestAtMs?: number | null;
  /* САМО ПОКАЗАНИЕ ПРИБОРА (rev ≥ 1.73, ядро #221).

     `latestAtMs` говорил, КОГДА пришло показание, и молчал о том, КАКОЕ. Из-за этого мы не могли ни
     посчитать расхождение приборов, ни отсортировать карточки по числу, ни нарисовать вторую ленту
     на графике. Ядро отдало его по нашей просьбе (#822) вместе с готовым расхождением: сырьё нужно
     нам шире, чем один этот случай. */
  latestGlucoseMmol?: number | null;
  autoConnect?: boolean;
...
export interface Discovered {
  ...
  /* ПРИБОР ДЕРЖИТ ДРУГАЯ ПРОГРАММА (rev ≥ 1.69, SugarLife#717).

     Найден не сканом, а системным списком подключённых: занятый прибор в эфир не рекламируется, и
     это единственный вход к нему. Отсюда и `busy` приезжает независимо от того, идёт ли скан, —
     ждать скана значило бы вернуть тот же тупик.

     Для человека это ДРУГОЙ РАЗГОВОР, а не пометка сбоку: «свободный — подключить» и «им уже
     пользуются» ведут к разным решениям, и подавать их одинаково нельзя. */
  busy?: boolean;
  /* УЗНАЛИ ЛИ ДРАЙВЕР (rev ≥ 1.69).

     `false` — честное «прибор рядом, занят, какой именно не знаем»: из системного списка известны
     только адрес и имя, а узнавание построено на service-UUID, которых там нет. Тогда `driverId`
     пуст, а `displayName` говорит это словами.

     Своего узнавания по имени здесь не заводим: у ядра оно одно на оба пути (эфир и системный
     список), и второе рядом разошлось бы с ним молча. */
  driverKnown?: boolean;
}
...
/* ОДНО СРЕДСТВО ВЫЖИВАНИЯ ГЛАЗАМИ КОНКРЕТНОГО ЧЕЛОВЕКА (rev ≥ 1.71, ядро #217, наш #813).

   Каталог у движка был написан давно и подробно — и не показывался никому: функция, делавшая из
   него фразу, месяц числилась мёртвым кодом. Не хватало пятого факта: КОМУ ИМЕННО ЧТО НУЖНО. Одна
   и та же вендорская защита у человека со своим сенсором убирает редкие провалы, а у того, кто
   следит за другим через интернет, — это разница между «работает» и «восемь минут».

   СПИСОК УЖЕ ОТФИЛЬТРОВАН движком по режиму работы: ненужное сюда не приходит вовсе. Дополнять его
   своими строками и решать, что показать, не надо — просить лишнее значит учить нажимать не глядя,
   и расплатимся мы же, когда попросим нужное. */
export interface PermissionView {
  /** Машинное имя средства: `Уведомления`, `ВендорскаяЗащита`, `Bluetooth`… Не наш нативный код. */
  id: string;
  /* КАК ЭТО ЗОВЁТСЯ ЧЕЛОВЕКУ (rev ≥ 1.72, ядро #217, наш #813).

     Поля сначала не было, и мы написали пять имён сами — «ВендорскаяЗащита» в списке не покажешь.
     Спросили; ядро завело поле с доводом, который снимает вопрос целиком: два автора у одного
     разговора это то, чего мы избегаем. Свои имена убрали.

     Название говорит, ЧТО СРЕДСТВО ДАЁТ, а не как зовётся системная настройка: путь до неё у
     каждой прошивки свой и меняется от версии к версии, а смысл — нет. */
  title: string;
  /** `Обязательно` | `Желательно`. `НеНужно` сюда не приходит: такое не просят. */
  need: string;
  /** `Выживание` | `Возможность`. Смешивать одно с другим — терять доверие к первому. */
  purpose: string;
  /** Где средство вообще существует: `Android`, `iOS`. */
  platforms: string[];
  /** Что средство даёт. Одинаково для всех. */
  gives: string;
  /** Что продолжает работать без него. Половина спокойствия человека — здесь, и прятать это нельзя. */
  withoutIt: string;
  /** Цена. null — цены нет. */
  price: string | null;
  /** Можем ли мы вообще узнать, включено ли. false — только спросить человека и поверить. */
  checkable: boolean;
  /** Почему это нужно ИМЕННО ЕМУ. null — общего описания хватает.
   *  Показывать РЯДОМ с `gives`, а не вместо: `gives` про средство, `whyYou` про него самого, и
   *  второе и есть разница между текстом, который пролистывают, и тем, после которого нажимают. */
  whyYou: string | null;
}

/* ПРИБОРЫ РАСХОДЯТСЯ (rev ≥ 1.73, ядро #221, наша просьба #822).

   `null` — молчим, и молчим гораздо чаще, чем говорим: два сенсора расходятся всегда, и почти всё
   это расхождение норма. Поле появилось — значит движок уже отсеял всё, что объясняется приборами:
   расхождение продержалось четверть часа, числа одного сорта и возраста, обоим приборам есть
   двенадцать часов.

   ЧИСЛО И ФРАЗА ПРИЕЗЖАЮТ ВМЕСТЕ, и это мы же и просили: половина — число без фразы — соблазнила
   бы написать свою, а «как минимум один ошибается, проверьте глюкометром» это медицинское
   высказывание, и второй его редакции быть не должно. */
export interface DivergenceView {
  /** На сколько расходятся. Для показа рядом с фразой, не вместо неё. */
  mmol: number;
  /** Готовая фраза движка. Своей поверх не пишем. */
  text: string;
  /** Чьи числа сравнивались — подсветить ровно эти карточки, а не все. */
  deviceIds: string[];
}
...
export interface GuardGapView {
  ...
  minutes: number;
  /* Почему нас не было. `network` появился в 1.69: обрыв связи едет сюда же, а не отдельной
     поверхностью — для человека это об одном и том же, сколько он был без присмотра. */
  why: 'reboot' | 'swiped' | 'killed' | 'network' | string;
  words: string;
}
...
export interface UiSnapshot {
  ...
  /* Приборы расходятся (rev ≥ 1.73). Отсутствует или null — говорить не о чем. */
  divergence?: DivergenceView | null;
  /* ЧТО ПРОСИТЬ У ЭТОГО ЧЕЛОВЕКА И КАКИМИ СЛОВАМИ (rev ≥ 1.71, ядро #217, наш #813).

     Слова готовые, решать нам нечего. Пусто или отсутствует — старое ядро: тогда остаётся прежний
     список, собранный из опроса системы (`показ/разрешения.ts`). */
  permissions?: PermissionView[];
  /* Есть ли у человека свой прибор: `СПриборами` | `БезПриборов` | `ПокаНеЗнаем` (rev ≥ 1.71).

     Не роль и не выбор: подключил сенсор — сменилось само. ЭКРАНА ВЫБОРА ЗАВОДИТЬ НЕ НАДО, движок
     уже подобрал по нему `permissions`. Нам это только для понимания, почему список именно такой. */
  workMode?: string;
  /* Одной фразой: разбудим этой ночью или нет (rev ≥ 1.71). Слова ядра — на двух платформах одни. */
  wakeSummary?: string;
...
export interface AccountView {
  id: string;
  /* ПОЛЕ ЗОВЁТСЯ `vendor`, А НЕ `providerId` (найдено сверкой по типам, #813).

     У нас тут стояло `providerId`, и оно не приходило никогда: движок с самого появления
     вендорских учёток шлёт `vendor`. Фильтр «учётки этого провайдера» сравнивал undefined со
     строкой и всегда возвращал пусто — раздел облачных учёток показывал бы ноль учёток при любой
     заведённой. Прежняя сверка этого не видела: имя `vendor` есть у поддерживаемой модели прибора,
     и однофамилец закрывал собой пропажу.

     `providerId` — имя поля в ИНТЕНТЕ входа (`linkAccount`), и там оно верное. Совпадение имён у
     разных вещей и увело. */
  vendor: string;
  displayName: string;
...
export type Intent =
  ...
  /* ДОКЛАД О СВЯЗИ, А НЕ ПИНОК (rev ≥ 1.69, ядро #213 и наш #805).

     Раньше это был сигнал «проснись, сеть вернулась». Теперь — доклад: что со связью сейчас
     (`internet`), сколько её не было (`offlineMs`) и идём ли мы через VPN (`vpnActive`).

     У `vpnActive` ТРИ состояния, и это решение ядра, которое стоит понимать. `true` — включён,
     `false` — точно выключен, поля нет — не знаем. На Android второе достижимо (`TRANSPORT_VPN`
     документирован), на iPhone нет: там мы узнаём туннель по именам интерфейсов, и «не нашли» не
     значит «нет». Оговорку ядро заложило в тип, а не в текст: тогда её нельзя потерять при
     следующей правке слов. */
  | { type: 'networkBack'; internet?: boolean; offlineMs?: number; vpnActive?: boolean } // rev ≥ 1.42
