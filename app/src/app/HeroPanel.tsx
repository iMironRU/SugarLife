...
import { предупредитьОСыром, МЕТКА_СЫРОГО } from '@/domain/сырое';
            ...
            {batteries.map((b) => {
              ...
              return (
                <span key={b.id} className={'hp-batt-item' + (наИсходе ? ' на-исходе' : '')}
                  ...
                  /* Мышиной подсказки здесь нет намеренно: на телефоне её не существует, и
                     ставить её значило бы повторить ошибку, которую чиним в этой же правке. */>
      ...
      <div className="hp-row">
        ...
        <button className="hp-circle" onClick={() => setTab(1)} aria-label="Глюкоза">
          ...
          <span className="hp-circle-inner">
            ...
            {/* Подсказки при наведении здесь больше нет (#702): `title` на телефоне не
                показывается никогда, и звёздочка оставалась знаком без сноски. Слова движка
                теперь строкой под кругом (ui/ОговоркиЧисел.tsx), а знак указывает, какое из
                двух чисел неполное — этого слова внизу не говорят. */}
            <span className={'hp-iob' + (инс.известен ? '' : ' is-unknown') + (инс.неполный ? ' неполон' : '')}>{iobText}{инс.неполный ? ' *' : ''}</span>
            ...
            {/* Сырое значение помечаем ПРЯМО У ЧИСЛА (SugarLifeCore#88): сенсор отдаёт
                ...
                смотрят сюда. */}
            {/* Пояснение к пометке «сырое» — тоже строкой под кругом, а не в `title` (#702). */}
