...
/* ЧАСТЬ СУТОК НА ПРОЦЕНТ — ВЫНЕСЕНО ИЗ РАЗМЕТКИ (#665).

   Этот расчёт жил внутри обработчика кнопки, внутри `.map()`, внутри условия, внутри разметки —
   четыре уровня вглубь. Пока он там жил, проверить его было нечем: тестом до него не добраться.
   А считает он ДОЗУ ИНСУЛИНА.

   ЗАХВАТЫВАЕТ И ЧАСТИЧНО ПОПАДАЮЩИЕ ИНТЕРВАЛЫ, и это надо знать. `segsIn` берёт всё, что хотя бы
   краем задевает часть суток: интервал 05:00–09:00 попадает и в ночь, и в утро. Нажав «утро +5 %»,
   человек меняет и кусок ночи.

   Поведение не меняем — оно было таким с самого начала, и менять его вслепую на экране, где
   правят базал, опаснее, чем оставить. Но теперь оно названо словами и закреплено тестом: если
   решим сделать иначе, тест покажет, что именно меняется. */
export const scalePart = (segs: Seg[], p: { a: number; b: number }, pct: number): Seg[] => {
  const внутри = new Set(segsIn(segs, p).map((x) => x.i));
  return segs.map((s, i) => (внутри.has(i) ? { ...s, v: roundRate(s.v * (1 + pct / 100)) } : s));
};
