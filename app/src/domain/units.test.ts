...
/* «инс. −0,0 ед» на экране — прожито на эмуляторе (#517). Остаток инсулина уходит в
   микроскопический минус, и `toFixed` честно печатает минус нуля; человек читает это как поломку. */
describe('минус нуля не бывает', () => {
  it('микроскопический минус печатается нулём', () => {
    expect(fmt(-0.004)).toBe('0,0');
    expect(fmt(-0)).toBe('0,0');
  });

  it('настоящий минус остаётся минусом', () => {
