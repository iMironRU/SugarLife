...
import { FOODS, searchFoods, personalFoods, CAT_ORDER, ключГруппы } from './foods';
...
/* Ключ группы — общий для журнала и справочника (#381). Пока он собирался в двух местах,
   имя блюда могло показаться в одном списке и не показаться в другом. */
describe('ключГруппы', () => {
  it('округляет углеводы шагом 5 — 54 и 56 это один обед', () => {
    expect(ключГруппы('Обед', 54)).toBe(ключГруппы('Обед', 56));
  });

  it('запись без типа получает тот же ключ, что и «Приём»', () => {
    expect(ключГруппы(undefined, 30)).toBe(ключГруппы('Приём', 30));
  });

  it('разный тип — разные группы при тех же граммах', () => {
    expect(ключГруппы('Завтрак', 40)).not.toBe(ключГруппы('Ужин', 40));
  });
});
