...
/* Непрерывные куски типового дня (SugarLife#156).

   `agp()` выбрасывает корзины, в которых показаний не было, — и соседние по списку точки
   могут отстоять друг от друга на часы. Соединить их линией значит нарисовать сахар,
   которого никто не измерял, ровно там, где сенсор молчал. А на графике врача это самое
   интересное место: провал в данных ночью и ровная линия ночью выглядели бы одинаково.

   Порог — чуть больше шага корзин: при 48 корзинах шаг ровно полчаса, и «полчаса плюс
   погрешность» ещё соседство, а час — уже дырка. */
export function отрезки(точки: AgpPoint[], шагЧ: number): AgpPoint[][] {
  const куски: AgpPoint[][] = [];
  for (const p of точки) {
    const текущий = куски[куски.length - 1];
    if (текущий && p.t - текущий[текущий.length - 1].t <= шагЧ) текущий.push(p);
    else куски.push([p]);
  }
  return куски;
}
