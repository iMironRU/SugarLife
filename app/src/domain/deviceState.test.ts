...
/* Настоящий движок или браузерный шим (SugarLife#256).
   ...
   Признак «есть хоть один драйвер» сломался в тот день, когда Nightscout-шим завёл
   собственный драйвер — сам Nightscout, ради формы с адресом и токеном. С этого момента
   в вебе появилась карточка «Подключить устройство» и вела в поиск, который не найдёт
   ничего никогда: эфира в браузере нет как класса. */
describe('живой ли движок', () => {
  const дрв = (kind: string) => ({
    id: kind, displayName: kind, kind, roles: [], settings: { parameters: [] }, available: true,
  });
  const снимокД = (drivers: unknown[]): UiSnapshot => ({
    ...snap([]), availableDrivers: drivers as UiSnapshot['availableDrivers'],
  });

  it('только сервисный драйвер — это шим, а не движок', () => {
    expect(движокЖивой(снимокД([дрв('service')]))).toBe(false);
  });

  it('есть драйвер железа — движок настоящий', () => {
    expect(движокЖивой(снимокД([дрв('service'), дрв('sensor')]))).toBe(true);
  });

  it('драйверов нет вовсе — движка нет', () => {
    expect(движокЖивой(снимокД([]))).toBe(false);
    expect(движокЖивой(null)).toBe(false);
  });
});

/* Часики в круге — про отсутствие свежего числа, а не про состояние источника (#358).

   С телефона: круг показывает часики и «догоняю…», а строкой ниже список показаний,
   самому свежему из которых минута. Это два ответа на один вопрос на одном экране, и
   пустым оказывается именно то место, по которому принимают решение о коррекции. */
describe('вид круга', () => {
  const мин = 60_000;
  const сн = (status: string) => ({ monitor: { status } } as unknown as UiSnapshot);

  it('источник догоняет, но свежее число есть — показываем число', () => {
    expect(видКруга(сн('Acquiring'), 1 * мин)).toBe('число');
    expect(видКруга(сн('Connecting'), 1 * мин)).toBe('число');
  });

  /* Защита, ради которой часики и заводились (SugarLifeCore#27): при подключении в круг
     лезет догрузка истории. У неё отметки времени старые — возраст её и не пускает. */
  it('догрузка истории к числу не проходит', () => {
    expect(видКруга(сн('Acquiring'), 40 * мин)).toBe('ждём');
  });

  it('числа нет вовсе — часики', () => {
    expect(видКруга(сн('Acquiring'), null)).toBe('ждём');
    expect(видКруга(сн('Connecting'))).toBe('ждём');
  });

  it('связи нет и свежего числа нет — прочерк', () => {
    expect(видКруга(сн('Disconnected'), null)).toBe('нет');
    expect(видКруга(сн('Disconnected'), 40 * мин)).toBe('нет');
  });

  /* Источник оборван, а число минутной давности всё-таки есть — значит его донёс кто-то
     другой (база, облако). Прятать его незачем: оно настоящее. */
  it('связи нет, но число свежее — показываем', () => {
    expect(видКруга(сн('Disconnected'), 1 * мин)).toBe('число');
  });

  it('порог не двигается: ровно на нём ещё свежее', () => {
    expect(видКруга(сн('Acquiring'), ОТСТАВАНИЕ_КРУГА_МС)).toBe('число');
    expect(видКруга(сн('Acquiring'), ОТСТАВАНИЕ_КРУГА_МС + 1)).toBe('ждём');
  });

  it('мост молчит о статусе — ведём себя как раньше', () => {
    expect(видКруга({} as UiSnapshot, null)).toBe('число');
  });
});
