...
import { фонИнсулина, подписьФона, БЕЗ_СРОКА_МС } from './longInsulin';
...
describe('подпись', () => {
  const часы = () => '22:10';
  const давность = () => '2 ч назад';

  it('срок известен — называем его', () => {
    const ф = { ед: 24, когдаМс: т, доМс: т + час, действует: true };
    expect(подписьФона(ф, часы, давность)).toMatch(/примерно до/);
  });

  it('срок вышел — говорим прямо, а не «до»', () => {
    const ф = { ед: 24, когдаМс: т, доМс: т - час, действует: false };
    expect(подписьФона(ф, часы, давность)).toMatch(/закончилось/);
  });

  it('срока нет — про срок ни слова', () => {
    const ф = { ед: 24, когдаМс: т, доМс: null, действует: null };
    const с = подписьФона(ф, часы, давность);
