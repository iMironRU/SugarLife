...
export async function notifyAppReady(): Promise<void> {
  ...
  /* ЗАПИСЬ ПЕРВОЙ, ПОТОМ ПЛАГИН (#828).
     ...
     Порядок здесь был обратный, и это стоило дня: пока вызов плагина не возвращался, до записи дело
     не доходило вовсе — инструмент молчал от той же поломки, которую искал.

     Сама поломка оказалась в другом месте, чем я думал трижды подряд: не в динамическом импорте и
     не в нативном вызове, а в том, что прокси плагина возвращали через обещание. Разбор — в
     `издание.ts` у `обновитель()`. */
  try {
    вЖурналДвижка('Debug', 'update', 'подъём: 1 — зову обновитель');
    const плагин = обновитель();
    вЖурналДвижка('Debug', 'update', `подъём: 2 — ${плагин ? 'плагин есть' : 'плагина нет'}`);
    await плагин?.notifyAppReady();
    вЖурналДвижка('Debug', 'update', 'подъём: 3 — подтверждён');
  } catch (е) {
...
/* ШАГ, КОТОРЫЙ МОЖЕТ ЗАВИСНУТЬ, ОБЯЗАН СКАЗАТЬ ОБ ЭТОМ (#828).

   Две попытки обновиться на телефоне владельца написали «беру плагин» и замолчали. Ни следующей
   строки, ни ошибки. Между ними три шага, и ДВА ИЗ НИХ МОЛЧАЛИВЫ: `обновитель()` может вернуть
   `null` (тогда мы выходили `return false` без единой записи), а любое из ожиданий может просто не
   вернуться — `catch` ловит отказ, но не бесконечность.

   Поэтому у каждого шага теперь срок. Вышел — говорим, какой именно шаг и сколько ждали. Молчание
   перестаёт быть возможным исходом. */
