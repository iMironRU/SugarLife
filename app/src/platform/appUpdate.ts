...
export async function notifyAppReady(): Promise<void> {
  if (!isNative) return;
  /* ЗАПИСЬ ПЕРВОЙ, И СО СВОИМ СРОКОМ (#828).

     Она стояла ПОСЛЕ ожидания плагина, и 8 сентября это стоило ещё одного слепого дня: приложение
     запустилось в 09:06, а строки «интерфейс запущен» в журнале не появилось. Причина та же, что и
     у самой загрузки: `обновитель()` — динамический импорт, и пока он не вернётся, следующая строка
     не выполнится.

     То есть инструмент, заведённый чтобы увидеть поломку, сам от неё и замолчал. Теперь запись
     идёт первой и не ждёт никого. */
  void назватьЖивойИнтерфейс();
  try {
    const плагин = await сроком('плагин при запуске', 20, обновитель());
...
export async function назватьЖивойИнтерфейс(): Promise<void> {
  ...
  try {
    const плагин = await сроком('плагин для опроса текущего', 20, обновитель());
...
export async function вернутьВстроенный(): Promise<boolean> {
  ...
  try {
    const плагин = await сроком('получение плагина', 20, обновитель());
