...
export async function notifyAppReady(): Promise<void> {
  if (!isNative) return;
  /* ЗАПИСЬ ПЕРВОЙ, ПОТОМ ПЛАГИН (#828).

     Порядок был обратный, и это стоило дня: пока вызов плагина не возвращался, до записи дело не
     доходило вовсе — инструмент молчал от той же поломки, которую искал. Разбор самой поломки — в
     `издание.ts` у `обновитель()`. */
  try {
    await обновитель()?.notifyAppReady();
  } catch (е) {
    вЖурналДвижка('Warn', 'update', `notifyAppReady не прошёл: ${(е as Error)?.message ?? е}`);
  }
}

/* КАКОЙ ИНТЕРФЕЙС НА САМОМ ДЕЛЕ ЗАПУЩЕН (#828).

   Владелец: «вот смотри как ставится, точнее не ставится новый интерфейс». Проверить это было
   НЕЧЕМ: в журнале нет ни строки о том, какой веб-слой работает, и установка кабелем от неудачной
   отличалась только на глаз, по экрану «О приложении».

   А отличие есть и оно неочевидное. После удачного `set()` приложение поднимается НЕ ИЗ ВСТРОЕННОГО
   бандла, а из скачанного: он побеждает то, что мы кладём кабелем. То есть можно поставить сборку
   с компьютера десять раз подряд и продолжать работать на старом бандле, скачанном когда-то по
   воздуху, — и никакого следа этого в журнале не будет.

   Пишем обе стороны: что собрано у нас и что считает текущим плагин. Расхождение и есть ответ. */
export async function назватьЖивойИнтерфейс(): Promise<void> {
  if (!isNative) return;
  /* СНАЧАЛА СИНХРОННЫЙ ВОПРОС, И ТОЛЬКО ПОТОМ ИМПОРТ (#828).

     Ожидание плагина не возвращается — это установлено: два срока по двадцать секунд истекли при
     запуске 8 сентября. Но «не возвращается» не говорит, ПОЧЕМУ. Два мира ответов, и лечение у них
     разное: либо нативный плагин не зарегистрирован (тогда Capacitor уходит в веб-подделку, и чинить
     надо сборку), либо он на месте, а виснет сам импорт куска (тогда чинить надо разбиение бандла).

     `Capacitor.isPluginAvailable` синхронен и импорта не требует — то есть ответит даже тогда, когда
     всё остальное молчит. Ровно этого свойства и не хватало всем прежним заходам. */
  try {
    ...
    вЖурналДвижка('Info', 'update',
      /* СБОРКА ВЕБ-СЛОЯ — В ЭТУ ЖЕ СТРОКУ (#828). Без неё журнал не отвечает на первый вопрос,
         который к нему приходят задавать: КАКОЙ интерфейс сейчас работает. Три дня подряд я
         выводил это косвенно — по тому, какие строки появились, а какие нет, — и каждый раз
         ошибался. */
      `интерфейс ${APP_BUILD}; платформа ${я?.getPlatform?.() ?? '—'}; плагин обновления зарегистрирован: ` +
  ...
  try {
    const плагин = обновитель();
    const где = await сроком('опрос текущего бандла', 15, Promise.resolve(плагин?.current()));
    вЖурналДвижка('Info', 'update',
      `интерфейс запущен: встроенный ${APP_BUILD}, плагин считает текущим ` +
      `${где?.bundle?.version ?? '—'} (id ${где?.bundle?.id ?? '—'}, ${где?.bundle?.status ?? '—'})`);
  } catch (е) {
    вЖурналДвижка('Warn', 'update', `не смог узнать текущий бандл: ${(е as Error)?.message ?? е}`);
  }
}

/* ВЕРНУТЬСЯ НА ВСТРОЕННЫЙ ИНТЕРФЕЙС (#828).

   Нужен, потому что скачанный бандл побеждает установку кабелем. Пока он активен, поставить новый
   интерфейс с компьютера невозможно: приложение поднимется из старого, и правка, которую владелец
   ждёт, до него просто не доедет.

   Это же и единственный выход, если бандл приехал сломанным: `reset()` без аргументов возвращает
   ровно то, что лежит внутри установленного приложения. */
export async function вернутьВстроенный(): Promise<boolean> {
  if (!isNative) return false;
  try {
    const плагин = обновитель();
    /* РАНЬШЕ ЗДЕСЬ БЫЛ МОЛЧАЛИВЫЙ ВЫХОД. `return false` без записи — и снаружи это неотличимо от
    ...
    вЖурналДвижка('Info', 'update', 'возвращаемся на встроенный интерфейс');
    await плагин.reset();
    await sendIntent({ type: 'reportStopping', reason: 'update' }).catch(() => {});
    await плагин.reload();
    return true;
  } catch (е) {
    вЖурналДвижка('Error', 'update', `возврат на встроенный не удался: ${(е as Error)?.message ?? е}`);
    return false;
  }
}
...
/* СКОЛЬКО УЖЕ СКАЧАНО (#423).
   ...
   и то же число. */
/* ПРИЗНАК «ИДЁТ ЗАГРУЗКА» — ЗДЕСЬ ЖЕ, А НЕ В СОСТОЯНИИ ЭКРАНА (#848).

   Он жил в `useState` самой «О приложении», и это давало неправду: человек уходил с экрана и
   возвращался — строка снова говорила «Обновить интерфейс», хотя загрузка шла. Нажать её в этот
   момент было можно, и что случилось бы при второй загрузке той же версии, мы не знали.

   Владелец просил «прервать»; прервать плагин не умеет (`cancelDelay()` — про отложенное
   применение, а не про загрузку). Поэтому обещаем не отмену, которой нет, а честную видимость:
   состояние одинаково видно везде, уйти и вернуться можно, второе нажатие не проходит. */
let ходЗагрузки: number | null = null;
let идётЗагрузка = false;
const ходПодписки = new Set<() => void>();

function известить(): void { ходПодписки.forEach((f) => f()); }
...
function идёт(в: boolean): void {
  идётЗагрузка = в;
  известить();
}

const ходOta = (): number | null => ходЗагрузки;
const идётOta = (): boolean => идётЗагрузка;

/* Два хука вместо одного с объектом: `useSyncExternalStore` сравнивает снимок по ссылке, и
   объект, собранный заново на каждый вызов, гнал бы перерисовку без конца. */
...
/** Идёт ли загрузка прямо сейчас. Видно из любого места, а не только там, где нажали. */
export function useЗагрузкаИдёт(): boolean {
  return useSyncExternalStore(
    (cb) => { ходПодписки.add(cb); return () => { ходПодписки.delete(cb); }; },
    идётOta, идётOta,
  );
...
/* СРОК — ТОЛЬКО ТАМ, ГДЕ ЖДУТ ПО-НАСТОЯЩЕМУ (#828).

   Заводился он против молчания: `catch` ловит отказ, но не бесконечность, и «висит» снаружи
   неотличимо от «тихо вышли».

   ЧЕМУ ОН НАУЧИЛ, И ЭТО ВАЖНЕЕ САМОГО СРОКА. Сначала я накрыл им ДВА шага одним именем — и он
   честно сообщил, что «не ответил плагин», когда на деле не отвечал следующий вызов. Три дня я
   чинил не то место, потому что верил подписи, а не факту. Имя срока обязано называть ровно один
   вызов, иначе оно врёт убедительнее, чем молчание.

   Поэтому здесь остались только настоящие ожидания: список бандлов, опрос текущего и загрузка.
   Получение плагина синхронно и в сроке не нуждается. */
async function сроком<T>(что: string, сек: number, дело: Promise<T>): Promise<T> {
  let таймер: ReturnType<typeof setTimeout> | undefined;
  try {
    return await Promise.race([
      дело,
      new Promise<never>((_, отказ) => {
        таймер = setTimeout(() => отказ(new Error(`${что}: не ответило за ${сек} с`)), сек * 1000);
      }),
    ]);
  } finally {
...
export async function применитьOta(б: ОтаБандл): Promise<boolean> {
  ...
  /* НУЛЯ БОЛЬШЕ НЕ СТАВИМ, И ЭТО ПОПРАВКА ПО ЖИВОМУ СЛУЧАЮ (#828).

     Здесь стоял `ход(0)` с доводом: «0 %» означает «началось», а пустота — «неизвестно». Довод
     звучал разумно и оказался неверным. Владелец обновил интерфейс, увидел «Скачиваю… 0 %» на всю
     загрузку, решил, что зависло, снял приложение — а оно, открывшись, сказало, что обновилось.

     Ноль, который не двигается, читается не как «началось», а как «встало». Пустота честнее: у неё
     в интерфейсе своя фраза — «Скачиваю и перезапускаю…», без числа, которое не растёт.

     Процент показываем ТОЛЬКО когда плагин прислал настоящее событие. Не прислал ни одного —
     значит мы про ход ничего не знаем, и врать нечем. */
  /* ВТОРОЙ ЗАГРУЗКИ НЕ ЗАВОДИМ (#848). Раньше признак жил в экране, и после ухода-возврата кнопка
     снова предлагала обновиться поверх идущей загрузки. Что делают две загрузки одной версии
     разом, мы не знаем и узнавать на живом телефоне не станем. */
  if (идётЗагрузка) return false;
  идёт(true);
  ход(null);
  /* ПЕРВАЯ СТРОКА — ДО ЛЮБОГО `await` (#828).

     Она стояла после `await обновитель()`, и это оказалось важно. `обновитель()` — динамический
     импорт плагина; повисни он, и мы не напишем НИЧЕГО, а на экране навсегда останется «Скачиваю и
     перезапускаю…». Ровно это владелец и видит: попытка была, приложение перезапускалось дважды,
     строк с тегом `update` в журнале ноль.

     Ноль строк — сам по себе улика, но она говорит «мы не дошли даже до плагина», а не «загрузка
     встала». Различить их можно только записью, сделанной раньше первого ожидания. */
  вЖурналДвижка('Info', 'update',
    `просят обновить интерфейс на ${б.build ?? б.version}; беру плагин`);
  let слушатель: { remove: () => void } | null = null;
  /* ПУТЬ ОБНОВЛЕНИЯ БЫЛ НЕВИДИМ ЦЕЛИКОМ (#828).

     В журнале с телефона за сутки — семнадцать тысяч строк и НИ ОДНОЙ про обновление: первая
     запись стояла перед самой перезагрузкой, а всё до неё — тишина. Когда владелец сказал «0 % и
     не двигается», смотреть было не на что: пришли ли события вообще, докачалось ли, чем кончилось.

     Теперь называем каждый шаг. Строки уходят В БАЗУ через движок, то есть переживают смерть
     веб-слоя, которая наступит в конце. */
  let событийХода = 0;
  try {
    const плагин = обновитель();
    if (!плагин) return false;
    /* ЧТО УЖЕ ЛЕЖИТ НА ТЕЛЕФОНЕ — ПЕРВЫМ ДЕЛОМ (#828).

       Бандл на сервере полтора мегабайта и качается за полсекунды, а загрузка стоит на нуле. Значит
       дело не в сети и не в файле. Первая версия, которую надо либо подтвердить, либо снять: этот
       бандл уже скачан прошлой попыткой и лежит здесь — целым или недокачанным, — и плагин на
       повторе той же версии встаёт.

       Своими руками ничего не удаляем, пока не увидим список: удалить чужое состояние вслепую
       значит потерять единственное свидетельство. */
    try {
      const лежат = await сроком('список бандлов', 15, плагин.list());
      вЖурналДвижка('Info', 'update',
        `на телефоне бандлов: ${лежат?.bundles?.length ?? 0}` +
        (лежат?.bundles ?? []).map((x) => ` · ${x.version}=${x.status}`).join(''));
    } catch (е) {
      вЖурналДвижка('Warn', 'update', `список бандлов не прочитался: ${(е as Error)?.message ?? е}`);
    }
    вЖурналДвижка('Info', 'update', `плагин взят; начинаю загрузку ${б.build ?? б.version}, ${б.url}`);
    слушатель = await плагин.addListener('download', ({ percent }: { percent?: number }) => {
      if (typeof percent !== 'number') return;
      /* Первое событие называем отдельно: по нему видно, шлёт ли плагин ход вообще. Именно этого
         знания и не хватило, чтобы отличить «загрузка встала» от «мы не умеем показать её ход». */
      if (событийХода === 0) вЖурналДвижка('Info', 'update', 'первое событие хода загрузки');
      событийХода += 1;
      ход(Math.max(0, Math.min(100, Math.round(percent))));
    });
    /* Загрузке срок щедрый: полтора мегабайта на плохой связи законно едут минуту. Но не
       бесконечный — «висит навсегда» это тоже исход, и о нём надо сказать. */
    const bundle = await сроком('загрузка бандла', 180,
      плагин.download({ url: б.url, version: б.version }));
    вЖурналДвижка('Info', 'update',
      `бандл скачан, событий хода было ${событийХода}`);
    ход(100);
    ...
    /* ОШИБКУ БОЛЬШЕ НЕ ГЛОТАЕМ МОЛЧА (#828). Здесь стоял пустой `catch`, и неудачное обновление не
       оставляло следа нигде: ни на экране, ни в журнале. Разбирать было нечего. */
    вЖурналДвижка('Error', 'update',
      `загрузка не удалась после ${событийХода} событий хода: ${(е as Error)?.message ?? е}`);
    /* Ход сбрасываем, иначе полоска замрёт на сорока процентах и будет врать, что
       загрузка продолжается. */
    ход(null);
    return false;
  } finally {
    слушатель?.remove();
    /* На удачном пути сюда мы попадаем уже после `reload()`, и снимать признак незачем — но и
       вредного в этом нет, а на всех прочих путях он обязан сняться. */
    идёт(false);
  }
}
...
export async function откудаБандл(): Promise<'встроен' | 'по воздуху' | null> {
  ...
  try {
    const с = await обновитель()?.current();
...
export async function вернутьсяНаВстроенныйЕслиСвежее(): Promise<boolean> {
  ...
  if (!isNative) return false;
  try {
    const где = await обновитель()?.current();
    ...
    if (!плагин) return false;
    вЖурналДвижка('Info', 'update',
      `встроенный ${п.build} свежее текущего ${APP_BUILD} — откатываемся на него`);
    await плагин.reset();
    await плагин.reload();
    return true;
  } catch (е) {
    /* МОЛЧАТЬ ЗДЕСЬ БОЛЬШЕ НЕЛЬЗЯ (#828).

       Стоял пустой `catch { return false }` с доводом «метода нет — нативная сборка старше
       паспорта, это не поломка». Довод верен ровно для одного случая из всех, а глушил он любые.

       Неделю эта защита не срабатывала, и никто не знал: она сравнивает встроенный бандл с текущим
       и откатывается, если встроенный новее, — то есть делает то самое, ради чего 7 сентября
       заводилась кнопка «Вернуть встроенный». Работала ли она хоть раз, сказать было нечем.

       `Debug`, а не `Warn`: отсутствие метода на старом нативе — обычное дело, и пугать им незачем.
       Но в журнале оно теперь есть. */
    вЖурналДвижка('Debug', 'update', `сверка со встроенным не вышла: ${(е as Error)?.message ?? е}`);
    return false;
  }
}
