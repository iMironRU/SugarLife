...
/** Класс: пока он висит, приложение прозрачно и под ним видно камеру. */
const КЛАСС_СКАНА = 'скан-идёт';

/* ПРОЗРАЧНЫХ «ПРОЗРАЧНО» ЗДЕСЬ ДВА, И НУЖНЫ ОБА (SugarLife#621).

   Плагин делает прозрачным САМ VIEW: `getWebView().setBackgroundColor(TRANSPARENT)` — это про то,
   чем webview залит как элемент нативной иерархии. Фон `html` — заливка ВНУТРИ страницы, и она
   рисуется поверх прозрачного view. Одно другого не отменяет.

   Класс висел только на `body`, и правила доводили до прозрачности `body`, `ion-app` и
   `.app-shell`. Корень оставался нашим тёмным `rgb(22, 24, 38)` — и он один и рисовался, потому
   что всё остальное к тому моменту скрыто через `visibility: hidden`. На экране телефона этот
   цвет неотличим от чёрного, и его приняли за подложку камеры. Доказано опытом на живом
   устройстве: прозрачность корня выставили в отладчике — картинка появилась немедленно.

   Вешаем на КОРЕНЬ ТОЖЕ, а не переписываем селекторы через `:has()`: он поддержан не везде, куда
   мы ставимся (Safari младше 15.4), и сканер сломался бы там же и так же.

   Снимать обязательно с обоих: забытая прозрачность корня — это невидимое приложение, и человек
   опишет такую поломку словами «всё пропало». */
function прозрачность(включить: boolean): void {
  for (const узел of [document.body, document.documentElement]) {
    узел.classList.toggle(КЛАСС_СКАНА, включить);
  }
}
