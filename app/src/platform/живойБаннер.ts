...
/* ПОДНЯТЬ БАННЕР СЕЙЧАС (#568, просьба владельца «как сделать кнопку показать»).
   ...
   Баннер, умерший в кармане, сам не встанет: система разрешает создавать живое уведомление только
   активному приложению. Мы пробуем при возвращении на передний план, но молча — и когда не выходит,
   человек видит включённый выключатель и пустой экран блокировки, а объяснения нет нигде.

   Отсюда две вещи, и обе обязательны. Кнопка — потому что нажатие происходит ровно тогда, когда
   попытка разрешена. И ОТВЕТ словами — потому что молчаливый отказ читается как поломка кнопки.

   Слова подбираем здесь, а не в нативе: натив отвечает своим коротким исходом, а человеку нужно
   знать, что делать дальше. «Нет числа» — это не ошибка баннера, это молчащий сенсор, и чинить
   надо его. */
export interface ИтогПодъёма { получилось: boolean; словами: string }

const СЛОВА: Record<string, ИтогПодъёма> = {
  'запущено': { получилось: true, словами: 'Готово — баннер на экране блокировки.' },
  'обновлено': { получилось: true, словами: 'Баннер уже висел — обновили.' },
  'выключено': { получилось: false, словами: 'Сначала включите «Сахар на экране блокировки».' },
  'не умеет': { получилось: false, словами: 'Этой системе живой баннер недоступен: нужен iOS 18.' },
  'нет снимка': { получилось: false, словами: 'Движок ещё не ответил — подождите показание.' },
  'нет числа': { получилось: false, словами: 'Показывать нечего: сахара сейчас нет. Дело в приборе, не в баннере.' },
};
...
export async function поднятьБаннер(): Promise<ИтогПодъёма> {
  ...
  try {
    ...
    /* Неизвестный исход — это отказ системы, и текст у него её собственный («Target is not
