...
import { взятьРазделыPro } from './издание';
...
/* ДВИЖОК СПРАШИВАЕМ, КТО ОН (#294, решение владельца).

   Сборка отвечает «есть ли код вовсе», движок — «умеет ли он подавать». Проверка здесь про второе:
   Pro-бандл поверх lite-движка не должен показывать пульт помпы. Кнопка, которой нечем исполнить
   команду прибору, вводящему инсулин, — худший вид пустого обещания.

   В Lite-сборке (а тесты идут именно в ней) список пуст при любом ответе движка, и это тоже правда,
   которую стоит сторожить: граница безопасности — сборка, а не условие на экране. */
describe('движок решает, можно ли, а не только сборка', () => {
  it('движок молчит — разделов Pro нет', async () => {
    expect(await взятьРазделыPro(null)).toEqual([]);
    expect(await взятьРазделыPro(undefined)).toEqual([]);
  });

  it('движок сказал lite — разделов Pro нет', async () => {
    expect(await взятьРазделыPro('lite')).toEqual([]);
  });

  /* В Lite-сборке ответ «pro» ничего не открывает: кода этих разделов в бандле нет. */
  it('в Lite-сборке даже ответ «pro» не открывает пульт', async () => {
    expect(await взятьРазделыPro('pro')).toEqual([]);
  });
});
