...
(function () {
  ...
  var просьбаОткажет = false;
  window.демо = {
    /** Просьба о доступе отвечает отказом — как на iOS, когда доступ уже дан (#710). */
    просьбаНеСработает: function () {
      просьбаОткажет = true;
      return 'следующая просьба о доступе ответит отказом';
    },
    мешает: function (код) {
    ...
    /* ЗВУК В МАШИНЕ — ТО САМОЕ КРАСНОЕ, КОТОРОЕ УЕЗЖАЛО ВНИЗ (наш #523).

       Предупреждение «перехват звука выключен, тревога может не прозвучать» стояло по важности
       ниже спокойных сообщений, а показываем мы разом три — и оно уезжало за предел совсем.

       Здесь состояние ставит только его: спокойного соседа в браузере взять неоткуда — «помпа на
       паузе» читается из нашей загрузки Nightscout, которой тут нет. Порядок пары проверен
       тестами (`domain/notices.test.ts`), а глазами здесь видно, что красное вообще показывается
       и стоит первым. */
    звукВМашине: function () {
      снимок.audio = {
        holdAudioAnchor: false, route: 'car',
        words: 'Перехват звука выключен вами. Система может выгрузить приложение, и тревога не прозвучит.',
      };
      разослать();
      return 'предупреждение про звук в машине показано';
    },
  ...
  window.SugarLifeBridge = {
    ...
    sendIntent: function (i) {
      ...
      if (i.type === 'requestScanPermissions') {
        /* ОТКАЗ НА ПРОСЬБУ — ТРЕТИЙ ИСХОД, И ЕГО НАДО ВИДЕТЬ (наш #710).

           На iOS системное окно поднимается не всегда: доступ мог быть уже дан, и тогда дело не в
           разрешении вовсе. Оболочка отвечает отказом со словами, а интерфейс обязан их показать —
           нажатие, после которого на экране ничего не меняется, читается как поломка.

           Включается `window.демо.просьбаНеСработает()`. */
        if (просьбаОткажет) {
          return Promise.resolve({ accepted: false, error: 'доступ к блютусу уже есть' });
        }
