...
public class SugarLifeBridgePlugin: CAPPlugin, CAPBridgedPlugin {
    ...
    private func решитьПоБаннеру(_ json: String?, поПросьбе: Bool) -> String {
        ...
        /* ЗНАЧОК И СВОДКА ЖИВУТ НЕ ВНУТРИ КАРТОЧКИ (SugarLife#655).

           Они обновлялись ВНУТРИ решения о живой карточке, в одной из его веток. Пока карточка была
           включена, это работало и выглядело безобидно.

           Вчера я выключил карточку и поставил выход в начале функции — и унёс с ней значок и
           сводку. Владелец: «банера нет, но нет числа на значке и всплывающих уведомлений».

           Тот же капкан ждал и человека, который просто выключил бы баннер выключателем: старый
           выход по `sl.live-banner` стоял там же. То есть беда была заложена до меня, я её только
           разбудил.

           Три поверхности — три бюджета и три причины жить. Общей у них ровно одна: число из
           снимка. Поэтому считаем число один раз, кормим значок и сводку БЕЗУСЛОВНО, и только
           после этого решаем судьбу карточки. */
