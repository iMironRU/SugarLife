...
final class SharedCentral: NSObject, CBCentralManagerDelegate {
    ...
    /* ПОДНИМЕТСЯ ЛИ СИСТЕМНОЕ ОКНО ЕЩЁ РАЗ (SugarLife#710).

       Третье состояние, которого мы движку не сообщали вовсе: «не спрашивали» и «отказал насовсем»
       выглядели одинаково — просто «доступа нет». Из-за этого интерфейс показывал кнопку
       «Разрешить» и человеку, который уже отказал, а такая кнопка не открывает ничего: диалог iOS
       даёт РОВНО ОДИН РАЗ за установку.

       `nil` — сказать нечего (старая система): пусть движок не знает, это честнее выдуманного
       «да», по которому он пообещал бы диалог. */
    var спроситьМожно: Bool? {
        guard #available(iOS 13.1, *) else { return nil }
        switch CBCentralManager.authorization {
        case .notDetermined: return true
        case .denied, .restricted: return false
        case .allowedAlways: return nil          // спрашивать не о чем — доступ уже есть
        @unknown default: return nil
        }
    }

    /* ПОДНЯТЬ СИСТЕМНОЕ ОКНО. Отдельного «спросить» у CoreBluetooth нет: окно показывается при первом
       обращении к менеджеру. Значит просьба — это создание менеджера, то есть первое же касание
       `shared`. Возвращаем, случилось ли обращение впервые: если менеджер уже жил, окна не будет. */
    @discardableResult
    func разбудить() -> Bool {
        let былоНеопределено = спроситьМожно == true
        _ = central                              // касание поднимает менеджер, а с ним и окно
        return былоНеопределено
    }
...
public class SugarLifeBridgePlugin: CAPPlugin, CAPBridgedPlugin {
    ...
    /* СКОЛЬКО РАЗ ПЛАГИН ПОДНИМАЛСЯ ЗА ЖИЗНЬ ПРОЦЕССА (SugarLife#696).

       30 августа обновление интерфейса убило приложение на шесть минут. Восстановить, ЧТО именно
       умерло — процесс целиком или только плагин с движком, — по журналу не вышло: со стороны
       натива в нём нет ни одной строки о собственном подъёме.

       Разница не косметическая, и лечится в разных местах. Поднялся плагин второй раз в том же
       процессе — виноваты мы: перезагрузка веб-слоя пересоздаёт плагин, а старый движок остаётся
       висеть, и в памяти их становится двое. Счётчик обнулился вместе с процессом — приложение
       убила система, и чинить надо не пересоздание, а причину выгрузки.

       Статическая переменная живёт ровно столько же, сколько процесс, и это здесь ровно то, что
       нужно измерить. */
    ...
    override public func load() {
        ...
        NSLog("SugarLife: плагин поднялся, раз #\(подъём) за жизнь процесса")
        ...
        engineQueue.async { [weak self] in
            ...
            let reportReadiness = { [weak e] in
                ...
                /* Только когда есть что сказать (SugarLife#710). Поля нет — движок не знает, и это
                   честнее выдуманного «да»: по нему он пообещал бы диалог, которого не будет. */
                if let ещёРаз = SharedCentral.shared.спроситьМожно { json += ",\"canAskAgain\":\(ещёРаз)" }
                json += "}"
            ...
            reportReadiness()
            self.engine = e
            /* Ту же весть — в журнал движка: NSLog переживает только консоль отладчика, а этот
               вопрос задают наутро по выгрузке. Пишем ПОСЛЕ создания движка — раньше некуда. */
            self.вЖурнал(подъём > 1 ? "Warn" : "Info", "native",
                         "плагин поднялся, раз #\(подъём) за жизнь процесса")
            ...
            /* Просьбы к системе перерисовать виджет — тем же путём (#692). Раньше их не было видно
               вовсе: мы звали систему полторы тысячи раз в сутки и не знали об этом ничего. */
            if #available(iOS 16.0, *) {
                ПоверхностьВиджета.общая.вЖурнал = { [weak self] уровень, событие in
                    self?.вЖурнал(уровень, "widget", событие)
                }
            }
    ...
    func оживитьБаннер(_ повод: String) {
        ...
        guard let json = последнийСнимок else { return }
        /* КАРТОЧКА ЖИВА — ВСЁ РАВНО КОРМИМ, И ЭТО ЗАМЕР (#428).

           Рождение работает только на переднем плане — это измерено. Работает ли оттуда же
           ОБНОВЛЕНИЕ, не знает никто: все наши обновления до сих пор уходили из фона, потому что
           показания приходят когда придут.

           Здесь единственное место, где мы гарантированно активны. Кормим карточку принудительно,
           мимо порогов, и журнал пишет, где это случилось. Открыл человек приложение — получили
           ответ на вопрос, который иначе пришлось бы ждать случая.

           Если обновления с переднего плана ложатся, а из фона нет, то модель «витрина на сутки»
           невозможна ни при каких порогах, и остаётся сеанс. Это решение, а не мелочь, и принимать
           его надо на замере. */
        if ЖивойБаннер.живой {
            показанноеПоказаниеМс = 0
            _ = обновитьЖивойБаннер(json, поПросьбе: true)
            return
        }
    ...
    /* Сказали ли уже, что карточка выключена. Один раз за сеанс — пояснение у самой развилки.
       Живёт ЗДЕСЬ, а не рядом с ней: там расширение под iOS 16.2, а хранимому свойству
       нельзя быть доступным по версии. */
    private var сказалиЧтоКарточкаВыключена = false
    /* С какого момента ждём переднего плана и сколько снимков прошло мимо (#428). Нужны, чтобы
       сказать об ожидании один раз, а о его длине — когда оно кончится. */
    private var ждёмПереднегоПлана: Date?
    private var пропущеноВФоне = 0
    ...
    /* Не чаще этого просим перерисовку — кроме случаев, когда смысл карточки изменился.
       ...
       ЧИСЛО ПЕРЕЕХАЛО В `СрокиКарточки` (#428). От него считается срок годности карточки, и пока
       оно жило здесь, а срок — там, они разошлись: запас считался от пятиминутного порога, а
       молчали мы по десятиминутному. Карточка тускнела между нашими же кормлениями. */
    private var баннерНеЧащеМс: Double { СрокиКарточки.порогПовтораСек * 1000 }
    ...
    private func обновитьЖивойБаннер(
        ...
        /* КАРТОЧКА ВЫКЛЮЧЕНА — ЭТО НЕ СОБЫТИЕ (SugarLife#682).

           «Выключено владельцем» писалось на каждый снимок: 2954 записи за сутки, почти треть
           всего журнала. И это была не только грязь — следом заводилась отложенная перепроверка,
           то есть фоновая работа ради карточки, которой по решению владельца нет вовсе. В фоне
           такая работа стоит жизни приложения, а мы за неё в эти сутки заплатили пятой частью.

           Сказать надо ОДИН раз: читающий журнал должен понимать, почему баннера нет, но узнаёт
           он это с первой строки, а не с двухтысячной. */
        if итог == "выключено владельцем" || итог == "выключено" {
            if !сказалиЧтоКарточкаВыключена {
                сказалиЧтоКарточкаВыключена = true
                вЖурнал("Info", "banner", итог + " — дальше об этом молчим", решение: "banner")
            }
            return итог
        }
        /* ЖДАТЬ ПЕРЕДНЕГО ПЛАНА — НЕ СОБЫТИЕ, А СОСТОЯНИЕ (#428, замер 3 сентября).

           После установки приложение прожило четырнадцать минут в фоне, и всё это время каждый
           снимок писал «рождение отложено: приложение не на переднем плане» — 32 строки, по две на
           снимок. Вторая приходила от отложенной перепроверки: она ждёт четыре секунды и спрашивает
           снова, хотя за четыре секунды приложение передним планом не станет.

           Та же болезнь, что была у «выключено владельцем» (2954 записи в сутки, #682), и лечение
           то же: сказать один раз, а сколько это длилось — сводкой, когда кончится. Перепроверку не
           заводим вовсе: ждать здесь надо не пачку догрузки, а человека, и разбудит нас не таймер,
           а `оживитьБаннер` при возвращении.

           Строку про ожидание пишем ОДИН раз, но снятие ожидания — обязательно: без него в журнале
           осталось бы «ждём переднего плана» без ответа, чем это кончилось. */
        if итог.hasPrefix("рождение отложено") {
            if ждёмПереднегоПлана == nil {
                ждёмПереднегоПлана = Date()
                пропущеноВФоне = 0
                вЖурнал("Debug", "banner", итог + " — дальше молчим, пока не вернётесь", решение: "banner")
            } else {
                пропущеноВФоне += 1
            }
            return итог
        }
        if let ждём = ждёмПереднегоПлана, итог == "запущено" || итог == "обновлено" {
            вЖурнал("Info", "banner", "ждали переднего плана \(Int(Date().timeIntervalSince(ждём))) с, "
                + "снимков мимо \(пропущеноВФоне)", решение: "banner")
            ждёмПереднегоПлана = nil
            пропущеноВФоне = 0
        }
    ...
    private func решитьПоБаннеру(_ json: String?, поПросьбе: Bool) -> String {
        /* РЕШЕНИЕ ВЛАДЕЛЬЦА — ПЕРВЫМ ВОПРОСОМ, А НЕ ОДНИМ ИЗ (SugarLife#691).

           Стояло ниже, за проверками «нет числа» и «без изменений», — и до него почти не
           доходило: показание чаще всего то же самое, и разбор возвращался на полпути. В журнале
           это дало 264 записи «без изменений» за три часа при одной «выключено владельцем»:
           молчание, которое мы вчера завели, обходилось стороной.

           Порядок здесь не вкусовщина. Выключенная карточка — самый дешёвый и самый окончательный
           ответ: он не зависит ни от снимка, ни от прошлого показания, и после него делать нечего.
           Стоя ниже, он заставлял разбирать JSON на каждый приходящий снимок ради того, чего по
           решению человека нет вовсе. */
        ...
        /* ЖДАНИЕ ПЕРЕДНЕГО ПЛАНА СЮДА НЕ ПОПАДАЕТ (#428).

           Здесь пишется КАЖДЫЙ исход — и это правильно: лог обязан отвечать на «выполнялось ли
           приложение в 3:40». Но «рождение отложено» уже сказано выше, один раз и со сводкой, а
           тут оно печаталось вторым голосом на каждый снимок.

           Я завёл глушилку в вызывающем и не заметил, что решение пишет о себе само. Два места
           пишут об одном исходе — та же беда, что мы весь месяц разбираем, только с обратным
           знаком: не молчание в двух смыслах, а двойная речь об одном.

           Проверить это можно было только на телефоне: в журнале строки идут подряд, а в коде
           лежат в разных функциях за двести строк друг от друга. */
        if !итог.hasPrefix("рождение отложено") {
    ...
    @objc func sendIntent(_ call: CAPPluginCall) {
        ...
        /* «РАЗРЕШИТЬ» — ТОЖЕ НАШЕ ДЕЛО, А НЕ ДВИЖКА (SugarLife#710).

           Интент перехватывался только на Android; здесь он уходил в движок, тот честно писал
           `intent-not-handled` — и всё. Кнопка не делала ничего, причём одна из двух её копий стоит
           на ЭКРАНЕ ЗНАКОМСТВА: у человека, который только поставил приложение и не выдал блютус,
           молчала самая первая кнопка.

           Отдельного «спросить» у CoreBluetooth нет: системное окно поднимается при первом обращении
           к менеджеру. Поэтому просьба — это создание менеджера. Три исхода, и все три отвечают
           словами, потому что нажатие, после которого на экране ничего не меняется, читается как
           поломка — это мы уже разбирали с жестом «потянуть».

           Отказавшему НАСОВСЕМ окно больше не покажут: iOS даёт его один раз за установку. Ему
           открываем настройки приложения — единственный экран, который система пускает открыть. */
        if json.contains("\"requestScanPermissions\"") {
            let ещёРаз = SharedCentral.shared.спроситьМожно
            if ещёРаз == true {
                SharedCentral.shared.разбудить()
                вЖурнал("Info", "native", "просим доступ к блютусу: поднимаем системное окно")
                return call.resolve(["json": "{\"accepted\":true}"])
            }
            if ещёРаз == false {
                вЖурнал("Info", "native", "доступ к блютусу отклонён насовсем — ведём в настройки")
                открытьНастройкиПриложения()
                return call.resolve(["json": "{\"accepted\":true}"])
            }
            /* Доступ уже есть (или система старая и сказать нечего). Просить нечего, и молчать
               нельзя: пусть интерфейс скажет, что дело не в разрешении. Заодно пересчитываем
               готовность — вдруг помеха уже ушла, а движок об этом не знает. */
            SharedCentral.shared.readinessHandler?()
            return call.resolve(["json": "{\"accepted\":false,\"error\":\"доступ к блютусу уже есть\"}"])
        }
