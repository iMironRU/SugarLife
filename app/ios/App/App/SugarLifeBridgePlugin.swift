...
public class SugarLifeBridgePlugin: CAPPlugin, CAPBridgedPlugin {
    ...
    private func обновитьЖивойБаннер(_ json: String?) -> String {
        ...
        /* История движка полнее нашей всегда, когда она есть: в ней нет дыр от сна приложения.
           Последнее показание дописываем из снимка — запрос мог отстать на минуту. */
        if !историяДвижка.isEmpty {
            var изДвижка = историяДвижка
            let момент = Date(timeIntervalSince1970: когдаМс / 1000)
            if изДвижка.last.map({ момент.timeIntervalSince($0.т) > 30 }) ?? true,
               let mmol = monitor["glucoseMmol"] as? Double, mmol > 0 {
                изДвижка.append(.init(т: момент, в: mmol))
            }
            ряд = изДвижка
        }
