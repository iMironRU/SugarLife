...
public class SugarLifeBridgePlugin: CAPPlugin, CAPBridgedPlugin {
        ...
        engineQueue.async { [weak self] in
            ...
            self.unsubscribe = e.subscribe(onSnapshot: { [weak self] json in
                ...
                DispatchQueue.main.async {
                    ...
                    self?.обновитьЗначокИСводку(json)
    ...
    /* ЗНАЧОК И ТИХАЯ СВОДКА. Своя память о показанном — намеренно отдельная от баннерной: общая
       означала бы, что решение одной поверхности молчит за другую, а это ровно то, что мы чиним.

       Своё условие «есть что менять»: пришло другое показание или сменилась свежесть. Без него
       проигрывание истории на старте (сотни снимков в секунду) перерисовывало бы значок сотни раз
       подряд ради одного и того же числа. */
    private var значокПоказаниеМс: Double = 0
    private var значокСтарое = false

    private func обновитьЗначокИСводку(_ json: String?) {
        guard let json, let data = json.data(using: .utf8),
              let корень = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
              let monitor = корень["monitor"] as? [String: Any] else { return }
        let значение = (monitor["glucose"] as? String) ?? "—"
        ...
        guard когдаМс != значокПоказаниеМс || старое != значокСтарое else { return }
        значокПоказаниеМс = когдаМс
        значокСтарое = старое
        ...
        Сводка.общая.обновить(
            ...
            стрелка: стрелкаТренда(monitor["trend"] as? String),
            разница: дельта(рядДляПоказа(monitor, когдаМс: когдаМс)),
