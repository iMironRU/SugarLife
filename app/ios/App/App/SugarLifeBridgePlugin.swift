...
public class SugarLifeBridgePlugin: CAPPlugin, CAPBridgedPlugin {
    ...
    /* ИНСУЛИН ДЛЯ ПОВЕРХНОСТЕЙ: СНАЧАЛА ДВИЖОК, ПОТОМ ВЕБ (#528, мост 1.42).

       Раньше единственным источником был веб-слой: он грузил `devicestatus` сам и присылал число
       сюда, пока приложение открыто. В фоне присылать было некому, и баннер молчал про инсулин
       ровно тогда, когда на него смотрят.

       Теперь чужой расчёт приходит в снимке (`loopIOB`), и он же виден на «Сегодня» — одно число на
       всех поверхностях. Наш `confirmedIOB` берём, если он есть: он построен на свидетельстве и
       вернее чужого. Веб остаётся последним запасным путём — для старого ядра и браузера. */
