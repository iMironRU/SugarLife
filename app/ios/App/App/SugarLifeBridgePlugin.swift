...
public class SugarLifeBridgePlugin: CAPPlugin, CAPBridgedPlugin {
    ...
    private func рядДляПоказа(_ monitor: [String: Any], когдаМс: Double) -> [СахарАтрибуты.ContentState.Точка] {
    ...
    /* Не чаще этого просим перерисовку — кроме случаев, когда смысл карточки изменился.
       ...
       Было пять минут — «раз в пять минут нормально, даже если показания приходят чаще». Владелец
       трижды за день сказал, что карточка не обновляется, и трижды был прав: показания приходят
       раз в минуту, а число на экране блокировки стояло по пять. Рядом чужое уведомление менялось
       каждую минуту, и разница читалась как поломка.

       ТЕХНИЧЕСКОЕ ОБОСНОВАНИЕ ПОРОГА БЫЛО НЕ ИЗМЕРЕНО. В комментарии стояло «система считает наши
       просьбы и лишние отбрасывает молча» — утверждение, которое мы никогда не проверяли: до
       сегодняшнего дня исход обновления не был виден вовсе, мы рапортовали об успехе до того, как
       он случался.

       Теперь виден: после каждого обновления пишем, каким карточку видит сама ActivityKit. Значит
       порог можно поставить по данным и посмотреть, что ответит система, — а не держать пятиминутку
       из осторожности, цена которой платится каждым взглядом на экран.

       Минута — такт источника. Чаще незачем: нового числа всё равно нет. */
    private let баннерНеЧащеМс: Double = 60 * 1000
    ...
    private func обновитьВиджет(_ json: String?) {
        ...
        let ряд = рядДляПоказа(monitor, когдаМс: когдаМс)
    ...
    private func решитьПоБаннеру(_ json: String?, поПросьбе: Bool) -> String {
        ...
        if !поПросьбе, ЖивойБаннер.живой, !смыслСменился, сейчасМс - баннерПросиливМс < баннерНеЧащеМс {
            ...
            Сводка.общая.обновить(
                ...
                инсулин: инсулинИзСнимка(monitor), когдаМс: когдаМс, старое: старое)
            return "не чаще"
        }
        ...
        Сводка.общая.обновить(
            сахар: число.isEmpty ? значение : число, стрелка: стрелка, разница: дельта(ряд),
            инсулин: инсулинИзСнимка(monitor),
     ...
     */
    private func зонаСахара(_ mmol: Double?) -> String {
    ...
    private func инсулинИзСнимка(_ monitor: [String: Any]) -> Double? {
     ...
     */
    private func дельта(_ ряд: [СахарАтрибуты.ContentState.Точка]) -> String {
