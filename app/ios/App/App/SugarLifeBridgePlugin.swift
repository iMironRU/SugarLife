...
public class SugarLifeBridgePlugin: CAPPlugin, CAPBridgedPlugin {
    ...
    private func обновитьИсториюИзДвижка() {
        ...
        /* maxPoints просим сразу: прореживать на стороне движка дешевле, чем тащить сотни точек
           через мост ради линии в пол-экрана шириной. */
        let запрос = "{\"kind\":\"Glucose\",\"fromMs\":\(Int(от)),\"toMs\":\(Int(сейчас)),\"maxPoints\":48}"
        guard let ответ = engine?.query(json: запрос),
              let data = ответ.data(using: .utf8),
              let корень = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
              let список = корень["glucose"] as? [[String: Any]] else { return }
    ...
    /// «6,1 в 9:24» — что было известно до разрыва. Крупно этого не показываем, но и не прячем.
    private func последнееПоказание(число: String, когдаМс: Double) -> String {
        let ф = DateFormatter()
        ф.locale = Locale(identifier: "ru_RU")
        ф.dateFormat = "H:mm"
        return число + " в " + ф.string(from: Date(timeIntervalSince1970: когдаМс / 1000))
    }
