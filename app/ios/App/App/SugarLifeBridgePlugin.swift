...
public class SugarLifeBridgePlugin: CAPPlugin, CAPBridgedPlugin {
    ...
    /* Не чаще этого просим перерисовку — кроме случаев, когда смысл карточки изменился.
       ...
       Пять минут — решение владельца и правило модели показа (docs/поверхности-показа.md):
       «раз в пять минут нормально, даже если показания приходят чаще». Оно же и техническое:
       система считает наши просьбы и лишние отбрасывает молча, поэтому чаще буквально значит реже. */
    private let баннерНеЧащеМс: Double = 5 * 60 * 1000
    ...
    /// Последний исход обновления баннера — чтобы писать в журнал смену, а не каждую минуту.
    private var показанныйИтог = ""

    /// Строка в журнал движка: его выгрузка — единственный способ прочитать это с телефона потом.
    private func вЖурнал(_ уровень: String, _ тег: String, _ событие: String) {
    ...
    private func решитьПоБаннеру(_ json: String?, поПросьбе: Bool) -> String {
        ...
        /* И В ЖУРНАЛ ДВИЖКА — ЧТОБЫ ЭТО МОЖНО БЫЛО ПРОЧИТАТЬ С ТЕЛЕФОНА (#559).
           ...
           Пишем только смену исхода: одинаковые «обновлено» каждую минуту засорили бы журнал, ради
           которого всё и делается. */
        if итог != показанныйИтог {
            показанныйИтог = итог
    ...
    /* ИНСУЛИН ДЛЯ ПОВЕРХНОСТЕЙ: ТО ЖЕ ЧИСЛО, ЧТО НА ЭКРАНЕ (#566).

       Здесь стоял обратный порядок — сначала наш `confirmedIOB`, потом чужой `loopIOB`, — и это
       разводило поверхности. Владелец снял экран, где в шторке «инс. 0,7 ед», а в круге на
       «Сегодня» — «инс. 2,1 ед» в ту же минуту. Оба числа настоящие: в шторку попадал наш расчёт,
       в круг — расчёт петли (`показ/чужаяПетля.ts`). Разошлись они втрое.

       Почему втрое, а не на волосок: наш расчёт строится на записанных лечениях, а временный базал
       приходит без ставки и длительности (`TreatmentRecord` их не несёт, ядро назвало это своим
       следующим шагом). На замкнутом цикле временным базалом подаётся большая часть инсулина —
       значит наш `confirmedIOB` систематически занижен, и будет занижен, пока поле не приедет.

       Пока так — берём чужой первым. Не потому что он лучше: он готовый и необъяснимый. Потому что
       он полный, и потому что человек сверяет шторку с кругом, а не с нашей моделью. Заниженное
       число про инсулин — ошибка в сторону опасности: по нему решают, колоть ли ещё.

       Наш остаётся запасным — для тех, у кого петли нет вовсе. Веб — последним, для старого ядра. */
