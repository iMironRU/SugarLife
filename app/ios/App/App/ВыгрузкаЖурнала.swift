...
/* ОТДАТЬ ЖУРНАЛ ЧЕЛОВЕКУ (SugarLife#666).
   ...
   Вычистку секретов начинает ядро — `Logger` рендерит значения полей через `Redactor`, — но она
   идёт по ИМЕНИ ПОЛЯ и адрес облака не ловит: он лежит в `id`, а полный запрос внутри текста
   ошибки. Поэтому перед записью в файл прогоняем `ВычисткаЖурнала` (#656).

   Дальше мы только складываем файл и показываем системный лист — отправляет человек, выбирая
   получателя. */
enum ВыгрузкаЖурнала {
    static func отдать(ndjson: String?, контроллер: UIViewController?,
        ...
        do { try (шапкаНатива + ВычисткаЖурнала.вычистить(ndjson)).write(to: url, atomically: true, encoding: .utf8) }
