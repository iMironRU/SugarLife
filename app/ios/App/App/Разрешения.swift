...
enum Разрешения {
    ...
    static func собрать(_ готово: @escaping ([[String: Any]]) -> Void) {
        UNUserNotificationCenter.current().getNotificationSettings { s in
            ...
            /* «Здоровье» — особый случай: система НЕ говорит, дали ли читать (по факту отказа
               можно было бы догадаться о диагнозе). Поэтому статус здесь всегда «неизвестно», а не
               «разрешено» или «нет»: выдумывать ответ там, где его не дают, нельзя. Спросить можно
               ровно один раз — дальше лист не покажется, и об этом честно говорит `спросить`. */
            if ЗдоровьеТелефона.доступно {
                список.append(Пункт(id: "здоровье", статус: .неизвестно, спросить: false))
            }
