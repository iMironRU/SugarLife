...
final class Громкость {
    ...
    /*
     Поднять, запомнив прежний уровень. Повторный вызов ничего не портит.

     НАСКОЛЬКО — решает движок (`raiseVolumeTo`, мост 1.35). Своего предела мы больше не назначаем:
     семьдесят процентов ночью и сто в кармане — разные решения, и принимать их должен тот, кто знает
     обстановку. Умолчание оставлено на случай, если поле не приехало.

     ТОЛЬКО ВВЕРХ. Если человек и так слушает громче — не трогаем: убавить чужую громкость ради
     тревоги значит сделать её тише.
     */
    func поднять(до уровень: Float = 1.0) {
        guard поднимаем, было == nil else { return }
        let текущая = сейчас
        guard уровень > текущая else { return }
        было = текущая
        выставить(min(max(уровень, 0), 1))
    }
