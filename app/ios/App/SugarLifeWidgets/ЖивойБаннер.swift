...
struct ГрафикСахара: View {
    ...
    /* Больше этого промежутка — не линия, а дыра: ровная линия через полчаса тишины читается как
       ...
       Пятнадцать минут — тот же порог, по которому показание считается протухшим
       (docs/поверхности-показа.md). Здесь стояло двенадцать, и это была третья цифра про одно и
       то же молчание: карточка уходила в разрыв на пятнадцатой минуте, а линия рвалась на
       двенадцатой — человек видел разорванный график при живом числе. */
    private static let РАЗРЫВ: TimeInterval = 15 * 60
    ...
    var body: some View {
        ...
        if состояние.разрыв {
            ...
            VStack(alignment: .leading, spacing: 4) {
                /* СЛОВО НАЗЫВАЕТ БЕДУ, А НЕ УВОДИТ ОТ НЕЁ (#573, правило 7).

                   Здесь стояло «Нет связи» на оба случая. Владелец увидел его на экране блокировки,
                   а строкой ниже, в нашей же сводке, лежало свежее число «сейчас» — то есть связь
                   была. Слово отправляло проверять сенсор, телефон и интернет вместо единственного
                   действия, которое помогает: открыть приложение, чтобы карточку снова покормили.

                   Ночью, спросонья, подсказка, уводящая чинить исправное, — худшая из возможных. */
                Text(состояние.голодная ? "Не обновляется" : "Данных нет")
