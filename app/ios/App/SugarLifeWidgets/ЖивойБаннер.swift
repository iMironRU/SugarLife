 ...
 */
@available(iOS 16.1, *)
/* СЧЁТЧИК НЕ ДОЛЖЕН ЧИТАТЬСЯ КАК ЧАСЫ.

   Здесь стоял `style: .timer`. Он рисует «13:08» — тринадцать минут восемь секунд с показания. Рядом
   иконка часов, рядом второй такой же счётчик «12:08», и всё это на экране блокировки, где настоящие
   часы висят прямо сверху.

   Владелец прочитал это как «сейчас 13:08, показание в 12:08» и решил, что баннер отстал на час.
   Прочитал бы так любой: два двоеточных числа под иконкой часов — это время суток, а не длительность.

   `.relative` пишет длительность словами («13 мин»), обновляется системой так же живо и не требует
   приложения. Точность до секунды тут не нужна вовсе: решение человека от восьми секунд не зависит,
   а вот перепутать час — меняет всё. */
        ...
        HStack(spacing: 10) {
            HStack(spacing: 5) {
                ...
                if состояние.разрыв {
                    ...
                    Text(состояние.когда, style: .relative).frame(maxWidth: размер * 5.4, alignment: .leading)
                }
            }
            ...
            if let следующее = состояние.следующее, !состояние.разрыв {
                HStack(spacing: 5) {
                    ...
                    if состояние.опоздание > 0 {
                        ...
                        Text(следующее, style: .relative).frame(maxWidth: размер * 5.4, alignment: .leading)
                    }
                }
    ...
    var body: some View {
        ...
        if состояние.разрыв {
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                /* И здесь тоже длительность словами, а не «13:08»: правило одно на всю карточку.
                   Подпись «Не обновляется» сверху подсказывает смысл, но полагаться на неё нельзя —
                   спросонья читают цифру, а не подпись. */
                Text(состояние.когда, style: .relative)
                    .font(.system(size: кегль * 0.57, weight: .light, design: .rounded))
                    .monospacedDigit()
                    .frame(maxWidth: кегль * 2.8, alignment: .leading)
                /* И ЧТО С ЭТИМ ДЕЛАТЬ — ТОЛЬКО КОГДА ГОЛОДНАЯ, И ТОЛЬКО ОДНО ДЕЙСТВИЕ.

                   Мы наконец знаем, почему карточка застревает, и знаем это фактом: после того как
                   систем­а выгружает приложение, оно поднимается в фоне и создать карточку заново НЕ
                   МОЖЕТ — ActivityKit разрешает это только приложению на переднем плане. За сегодня
                   таких отказов сорок один: «Target is not foreground». Карточка на экране в эти часы
                   — труп предыдущей, и наш же срок годности честно помечает её «Не обновляется».

                   Починить это изнутри нельзя: правило платформы. Единственное, что возвращает
                   карточку к жизни, — человек открывает приложение. Значит это и надо написать, а не
                   оставлять его гадать, что сломалось.

                   Только при `голодная`. Когда молчит прибор, открывать приложение бесполезно, и
                   советовать это значило бы отправлять чинить исправное — та самая ошибка, из-за
                   которой отсюда убрали «Нет связи». */
                if состояние.голодная {
                    Text("откройте приложение")
                        .font(.system(size: кегль * 0.28, weight: .medium))
                        .opacity(0.75)
                }
            }
...
struct КомпактныйБаннер: View {
    ...
    /* ВИД ДЛЯ МАШИНЫ — ПО МАКЕТУ ВЛАДЕЛЬЦА, А НЕ ПО МОЕМУ УСМОТРЕНИЮ (#591).

       Здесь стояла одна строка: число, стрелка, возраст. График, инсулин и «до следующего» я выбросил,
       написав в комментарии, что на приборной панели нет высоты. Это было неправдой: в макете CarPlay
       занимает 320 × 160 pt, и график там 252 × 60 — места хватает с запасом.

       Владелец сказал прямо: «мы говорили вписать в размер, а не изменить». Он прав. Настоящая
       трудность была в том, что прежняя раскладка переполнялась, и я решил её выбрасыванием — самый
       дешёвый способ, и он же самый неверный: выбрасывает не лишнее, а то, что труднее уместить.

       Макет (README пакета, раздел «5. CarPlay»): число 54 pt weight 300, стрелка 20; возраст и
       секундомер 14 pt прижаты вправо; график 252 × 60, линия 2,4; инсулин 14 pt внизу слева.

       Числа кегля берём из макета как есть. Уместиться помогает `minimumScaleFactor`, а не изъятие. */
    var body: some View {
        let вид = Вид(состояние)
        VStack(alignment: .leading, spacing: 8) {
            HStack(alignment: .firstTextBaseline, spacing: 8) {
                if состояние.разрыв {
                    /* В разрыве число не показываем крупно — оно уже ничего не значит; вместо него
                       причина и счётчик, как на большом баннере. */
                    Text(состояние.голодная ? "Не обновляется" : "Данных нет")
                ...
                Spacer(minLength: 6)
                /* Возраст и «до следующего» — те же две величины, что на экране блокировки, тем же
                   кеглем макета. Возраст в машине важнее всего: число человек видит боковым зрением
                   и не проверяет, а решение по нему может принять на светофоре. */
                ПараВремён(состояние: состояние, размер: 14)
            }
            /* ВПИСЫВАЕМСЯ, А НЕ ВЫБРАСЫВАЕМ. Настоящий размер семейства решает система, а не макет:
               320 × 160 — это то, из чего исходил дизайнер, и полагаться на него как на факт значит
               повторить прежнюю ошибку с другой стороны. Прошлый раз я выбросил график насовсем,
               потому что раскладка переполнялась.

               `ViewThatFits` перебирает варианты сверху вниз и берёт первый, который влез: полный
               макет; без инсулина; только график поменьше. Уступаем по одному и в порядке важности —
               инсулин переживёт отсутствие, а число и возраст остаются всегда, они в верхней строке. */
            ViewThatFits(in: .vertical) {
                VStack(alignment: .leading, spacing: 8) {
                    ГрафикСахара(состояние: состояние).frame(height: 60)
                    if !состояние.инсулин.isEmpty {
                        ИнсулинНаБорту(текст: состояние.инсулин, размер: 14)
                    }
                }
                VStack(alignment: .leading, spacing: 6) {
                    ГрафикСахара(состояние: состояние).frame(height: 44)
                    if !состояние.инсулин.isEmpty {
                        ИнсулинНаБорту(текст: состояние.инсулин, размер: 12.5)
                    }
                }
                ГрафикСахара(состояние: состояние).frame(height: 40)
                ГрафикСахара(состояние: состояние).frame(height: 26)
                /* Совсем нет места — пусто: верхняя строка с числом и возрастом уже сказала главное. */
