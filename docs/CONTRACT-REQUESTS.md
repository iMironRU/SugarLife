...
## Ждём от ядра (спрошено 11.08.2026)

- **Полный спек 1.7.** Без него мы не можем прочитать то, что движок уже отдаёт: `Monitor.source`,
  `DeviceInfo.{status, latestAtMs, autoConnect, note}`, `Intent.setAutoConnect`, `addCloudSource`.
  Приведём `bridge.ts` к 1.7 одним коммитом, дальше 1.8 ляжет сверху.
- Ответы по трём нестыковкам ниже.
